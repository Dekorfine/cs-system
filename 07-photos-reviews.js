...
const ReviewsModule = ({ user, employees, toast }) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const list = await prReviewsList({ order:{col:'created_at', asc:false}, limit:500 });
  ...
  const handleDelete = async (id) => {
    if (!await wsConfirm('确定要删除此评价任务？删除后无法恢复。')) return;
    const r = reviews.find(x => x.id === id);
    if (!r) return;
    const res = await prReviewUpsert({ ...r, deleted:true, updated_at: new Date().toISOString() });
  ...
  const handleClaim = async (review) => {
    ...
    const res = await prReviewUpsert(payload);
    if (res) { toast('✓ 已接单！记得完成后点"标记完成"'); load(); }
  };
  
  const handleUnclaim = async (review) => {
    if (!await wsConfirm('放弃这个任务？其他人将可以接')) return;
    const payload = {
      ...review,
      claimed_by: null,
      ...
      <div style={{padding:'12px 16px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, flexWrap:'wrap'}}>
          {review.product_image && (
            <img src={review.product_image} alt="" onClick={() => wsOpenImg(review.product_image)} title="点击看大图"
              style={{width:54, height:54, objectFit:'cover', borderRadius:8, border:'1px solid var(--line)', cursor:'zoom-in', flexShrink:0}} />
          )}
...
const ReviewEditor = ({ review, user, employees, onClose, onSaved, toast }) => {
  ...
  const [batchCount, setBatchCount] = useState(10);  // 🆕 默认 10(没特别要求就 10,常用 5/10/15;不要 1)
  const [images, setImages] = useState(review?.product_image ? [{ id: 'pi0', url: review.product_image, name: '产品图', kind: 'image', mime: 'image/jpeg' }] : []);  // 🆕 fix131 产品主图(上传/粘贴/抓取)
  const [fetchingImg, setFetchingImg] = useState(false);
  const pullProductImg = async () => {
    if (!productUrl.trim()) { alert('请先填产品链接'); return; }
    setFetchingImg(true);
    try {
      const u = new URL(productUrl.trim());
      const handle = (u.pathname.split('/products/')[1] || '').split(/[/?#]/)[0];
      if (!handle) throw new Error('链接里找不到 /products/ 句柄');
      const r = await fetch(u.origin + '/products/' + handle + '.js', { headers: { 'Accept': 'application/json' } });
      if (!r.ok) throw new Error('HTTP ' + r.status);
      const p = await r.json();
      let img = p.featured_image || (p.images && p.images[0]) || (p.media && p.media[0] && p.media[0].src);
      if (img && img.indexOf('//') === 0) img = 'https:' + img;
      if (!img) throw new Error('未找到产品图');
      setImages([{ id: 'pi' + Date.now(), url: img, name: (p.title || 'product') + '.jpg', kind: 'image', mime: 'image/jpeg', from_url: true }]);
  ...
  const handleSubmit = async () => {
    ...
    if (isEdit) {
      ...
      const needCount = Math.max(1, Math.min(50, parseInt(batchCount) || 1));
      const rid = 'rv-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
      const row = {
        id: rid,
        product_name: productName.trim() || null,
        product_url: productUrl.trim(),
      ...
      try {
        await prReviewInsert([row]);
        toast('✓ 已发布评价任务 · 需 ' + needCount + ' 条评价');
        onSaved();
      } catch (e) { alert('发布失败(写入共享库 product_reviews): ' + (e.message || e)); }
    }
    setSaving(false);
  };
    ...
    <div onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{padding:18, overflow:'auto', flex:1}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12}}>
            ...
            <div>
              ...
              <select value={site} onChange={e => setSite(e.target.value)}
                ...
                {REVIEW_DOMAINS.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
          </div>
          ...
          <div style={{marginBottom:12}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4}}>
              <label style={{fontSize:11, fontWeight:600, color:'var(--ink-2)'}}>产品图 <span style={{fontWeight:400, color:'var(--ink-4)'}}>(抓取 / 上传 / 粘贴,美工评价时参考)</span></label>
              <button type="button" onClick={pullProductImg} disabled={fetchingImg}
                style={{padding:'4px 10px', fontSize:11, fontWeight:600, border:'none', borderRadius:6, cursor:'pointer', background:'#0071e3', color:'white'}}>
                {fetchingImg ? '抓取中…' : '🔄 抓取产品图'}
              </button>
            </div>
            <MultiImageUploader attachments={images} setAttachments={setImages} />
          </div>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:12}}>
            ...
            {!isEdit && (
              <div>
                <label style={{fontSize:11, fontWeight:600, color:'var(--ink-2)', display:'block', marginBottom:4}}>需要几条评价 <span style={{fontWeight:400, color:'var(--ink-4)'}}>· 没特别要求就填 10,常用 5/10/15</span></label>
        ...
        <div style={{padding:'12px 18px', borderTop:'1px solid var(--line)', display:'flex', justifyContent:'space-between', alignItems:'center', gap:8}}>
          <div style={{fontSize:10, color:'var(--ink-3)'}}>
            {!isEdit && batchCount > 1 && <>💡 这条任务需要 <strong>{batchCount}</strong> 条评价</>}
          </div>
...
const ReviewCompleteModal = ({ review, user, onClose, onSaved, toast }) => {
  ...
  const handleSubmit = async () => {
    if (!workNotes.trim() && attachments.length === 0 && !reviewText.trim()) {
      if (!await wsConfirm('没填写任何工作详情或上传截图,确定标记完成吗?')) return;
    }
    setSaving(true);
    const now = new Date().toISOString();
    const userName = user.name + (user.alias ? ' ' + user.alias : '');
