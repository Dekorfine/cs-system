...
const ChargebacksModule = ({ user, employees, toast, focusId }) => {
  const [list, setList] = useState([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState(null);
  // 🆕 fix145: 从提醒/搜索跳来 → 等数据加载好,找到该条 → 自动打开编辑;useRef 防重复弹开
  const cbFocusRef = useRef(null);
  useEffect(() => {
    if (focusId && focusId !== cbFocusRef.current) {
      const cb = (list || []).find(x => x.id === focusId);
      if (cb) { cbFocusRef.current = focusId; setEditing(cb); }
    }
  }, [focusId, list]);
  ...
  const [cbViewMode, setCbViewMode] = useState(() => { try { return localStorage.getItem('cb_view_mode') || 'list'; } catch (e) { return 'list'; } });
  const setCbView = (m) => { setCbViewMode(m); try { localStorage.setItem('cb_view_mode', m); } catch (e) {} };
  // 🆕 时间筛选
  ...
  return (
    <div className="space-y-4 fade-in">
      <div className="paper rounded-2xl p-4">
        ...
        <div style={{marginTop:8, display:'flex', alignItems:'center', gap:12, flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex', alignItems:'center', gap:0, border:'1px solid var(--line)', borderRadius:6, overflow:'hidden'}}>
            <button onClick={() => setCbView('list')} title="列表视图"
              style={{padding:'4px 10px', fontSize:11, border:'none', cursor:'pointer', fontWeight:600, background: cbViewMode==='list'?'#0071e3':'white', color: cbViewMode==='list'?'white':'var(--ink-2)'}}>☰ 列表</button>
            <button onClick={() => setCbView('grid')} title="网格视图"
              style={{padding:'4px 10px', fontSize:11, border:'none', cursor:'pointer', fontWeight:600, background: cbViewMode==='grid'?'#0071e3':'white', color: cbViewMode==='grid'?'white':'var(--ink-2)'}}>▦ 网格</button>
          </div>
        </div>
      ...
      <AmountSummaryWidget
        ...
        getSite={c => c.site || c.website || wsOrderSite(c.order_no)}
      ...
      {loading ? (
        ...
        <div className={cbViewMode === 'grid' ? '' : 'space-y-2'}
          style={cbViewMode === 'grid' ? {display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(360px,1fr))', gap:12} : undefined}>
          {filtered.map(cb => (
...
const ChargebackCard = ({ cb, employees, user, isAdmin, onEdit, onDelete, onReload, toast }) => {
  ...
  const [loadedEvidence, setLoadedEvidence] = useState(null);
  useEffect(() => {
    if (!expanded || loadedEvidence !== null || Array.isArray(cb.evidence_files)) return;
    let alive = true;
    (async () => {
      try {
        const { data } = await CLOUD.client.from('chargebacks').select('evidence_files').eq('id', cb.id).maybeSingle();
        if (alive) setLoadedEvidence((data && data.evidence_files) || []);
      } catch (e) { if (alive) setLoadedEvidence([]); }
    })();
    return () => { alive = false; };
  }, [expanded]);
  ...
  const evidence = Array.isArray(cb.evidence_files) ? cb.evidence_files : (loadedEvidence || []);
  ...
  const toggleFault = async () => {
    const next = !cb.cs_fault;
    const res = await CLOUD.upsert('chargebacks', { ...cb, cs_fault: next, cs_fault_emp: next ? (cb.cs_fault_emp || cb.created_by) : null, updated_at: new Date().toISOString() });
    if (res) { toast(next ? '⚠️ 已标记客服失误 · 绩效 -3' : '已取消客服失误标记'); onReload && onReload(); }
  };
  ...
  return (
    <div className="paper rounded-2xl fade-in" style={{
      ...
      <div style={{padding:'12px 16px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, flexWrap:'wrap'}}>
          <div style={{flex:'1 1 240px', minWidth:200}}>
            ...
            <div style={{fontSize:15, fontWeight:600, marginBottom:3}}>
              <OrderRefLink orderNo={cb.order_no} /> · 💰 {cb.currency || 'USD'} {cb.amount || 0}
            </div>
            ...
            <div style={{fontSize:11, color:'var(--ink-3)', marginTop:6, display:'flex', alignItems:'center', gap:6}}>
              <span>产品图</span>
              <OrderProductThumb orderNo={cb.order_no} onPreview={(u)=>setPreviewFile({url:u, name:'产品图', mime:'image/jpeg'})} />
            </div>
            <div style={{fontSize:11, color:'var(--ink-3)', marginTop:8, display:'flex', gap:8, flexWrap:'wrap', alignItems:'center'}}>
              <span style={{display:'inline-flex', alignItems:'center', gap:7, padding:'4px 12px', borderRadius:9, background: urgency?.bg || '#f5f5f7', border:`1.5px solid ${(urgency?.color || '#d1d1d6')}`}}>
                <span style={{fontSize:11, color:'var(--ink-3)', fontWeight:600}}>⏳ 截止</span>
                <strong style={{fontSize:18, fontWeight:800, color: urgency?.color || 'var(--ink)', letterSpacing:'-.01em', lineHeight:1}}>{cb.deadline || '—'}</strong>
                {urgency && <span style={{fontSize:11, fontWeight:700, color: urgency.color}}>· {urgency.label}</span>}
              </span>
              <span>📅 拒付开始 {cb.opened_at}</span>
          ...
          <div style={{display:'flex', gap:4, flexWrap:'wrap', alignItems:'flex-start', flexShrink:0}}>
            {cb.status === 'pending' && (
            ...
            {isAdmin && (
              <button onClick={toggleFault} title="标记/取消「客服失误」· 计入绩效奖惩 -3/次"
                style={{padding:'5px 10px', borderRadius:5, cursor:'pointer', fontSize:11, fontWeight:600,
                  background: cb.cs_fault ? '#fef2f2' : 'white', color: cb.cs_fault ? '#dc2626' : 'var(--ink-3)',
                  border:'1px solid ' + (cb.cs_fault ? '#fca5a5' : 'var(--line)')}}>
                {cb.cs_fault ? '⚠️客服失误✓' : '标记失误'}
              </button>
            )}
        ...
        {/* 🆕 fix143: 流程步骤条 受理→处理→完成 */}
        {(() => {
          const handler = (cb.assigned_to_names || []).join(' / ');
          const steps = [
            { id:'recv', label:'受理', icon:'📥', who: cb.created_by_name, when: fmtStepDate(cb.opened_at || cb.created_at), done: !!cb.created_by_name && !!(cb.opened_at || cb.created_at), color:'#2563eb' },
            { id:'proc', label:'处理', icon:'🔧', who: handler, when: fmtStepDate(cb.assigned_at), done: !!handler && !!cb.assigned_at, color:'#9333ea' },
            { id:'done', label:'完成', icon:'✅', who: handler, when: fmtStepDate(cb.resolved_at), done: ['won','lost','closed'].includes(cb.status) && !!cb.resolved_at, color:'#16a34a' },
          ];
          return <div style={{marginTop:10, paddingTop:10, borderTop:'1px dashed var(--line)'}}><ProcessStepper steps={steps} /></div>;
        })()}
...
const ChargebackEditor = ({ cb, user, employees, onClose, onSaved, toast }) => {
  ...
  const [evidence, setEvidence] = useState(Array.isArray(cb?.evidence_files) ? cb.evidence_files : []);
  ...
  const [cbPull, setCbPull] = useState({ loading: false, products: null });
  const cbDoPull = async () => {
    const on = (orderNo || '').trim();
    if (!on) { alert('请先填订单编号'); return; }
    setCbPull({ loading: true, products: null });
    try {
      const v = await wsFetchOrderProducts(on);
      setCbPull({ loading: false, products: (v && v.products) || [] });
      if (v) {
    ...
    } catch (e) { setCbPull({ loading: false, products: [] }); }
  };
  const cbIsImgSaved = (p) => (evidence || []).some(a => (a.url || a.dataUrl) === p.image_url);
  const cbMakeImg = (p) => ({ id: 'a_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7), url: p.image_url, label: '产品图', kind: 'image', mime: 'image/jpeg', name: (p.title || 'product') + '.jpg', from_order: true, uploaded_at: new Date().toISOString() });
  const cbPickImg = (p) => {
    if (!p.image_url) return;
    if (cbIsImgSaved(p)) setEvidence((evidence || []).filter(a => (a.url || a.dataUrl) !== p.image_url));
    else setEvidence([...(evidence || []), cbMakeImg(p)]);
  };
  // 🆕 fix127: 整单拒付 = 一键加入全部灯的图;单品拒付 = 单独点某个灯
  const cbAddAll = () => {
    const add = (cbPull.products || []).filter(p => p.image_url && !cbIsImgSaved(p)).map(cbMakeImg);
    if (add.length) setEvidence([...(evidence || []), ...add]);
  };
  const cbClearOrderImgs = () => setEvidence((evidence || []).filter(a => !a.from_order));
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:'flex', gap:6}}>
              <input value={orderNo} onChange={e => setOrderNo(e.target.value)} autoFocus
                ...
                style={{flex:1, minWidth:0, padding:'8px 12px', border:'1px solid var(--line)', borderRadius:7, fontSize:13, fontFamily:'monospace'}} />
              <button type="button" onClick={cbDoPull} disabled={cbPull.loading} title="拉取订单:自动填客户邮箱/金额/币种,并可选产品图入证据"
                style={{whiteSpace:'nowrap', padding:'8px 12px', border:'none', borderRadius:7, background:'#0071e3', color:'white', fontSize:12, fontWeight:600, cursor:'pointer', flexShrink:0}}>
                {cbPull.loading ? '拉取中…' : '🔄 拉取订单'}
              </button>
            </div>
            {cbPull.products && (
              <div style={{marginTop:8, padding:8, background:'#f8fafc', border:'1px solid var(--line)', borderRadius:8}}>
                {cbPull.products.length === 0 ? (
                  <div style={{fontSize:11, color:'var(--ink-4)', display:'flex', justifyContent:'space-between'}}>
                    <span>没拉到该订单的产品(已尽量填客户/金额)</span>
                    <button type="button" onClick={() => setCbPull({loading:false, products:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11}}>收起</button>
                  </div>
                ) : (
                  <>
                    <div style={{fontSize:12, color:'var(--ink-2)', marginBottom:8, lineHeight:1.6}}>
                      拉到 <strong>{cbPull.products.length}</strong> 个产品 · 选择要附到拒付的灯图:
                      <div style={{fontSize:11, color:'var(--ink-4)', marginTop:2}}>
                        🟢 <strong>整单拒付</strong> → 点「全部加入」&nbsp;&nbsp;｜&nbsp;&nbsp;🔵 <strong>某个产品拒付</strong> → 只点那盏灯的「加入」
                      </div>
                    </div>
                    <div style={{display:'flex', gap:8, marginBottom:10, flexWrap:'wrap'}}>
                      <button type="button" onClick={cbAddAll}
                        style={{padding:'6px 14px', border:'none', borderRadius:7, background:'#16a34a', color:'white', fontSize:12, fontWeight:700, cursor:'pointer'}}>➕ 全部加入(整单拒付)</button>
                      {(evidence || []).filter(a => a.from_order).length > 0 && (
                        <button type="button" onClick={cbClearOrderImgs}
                          style={{padding:'6px 12px', border:'1px solid var(--line)', borderRadius:7, background:'white', color:'var(--ink-3)', fontSize:12, cursor:'pointer'}}>清空已加入</button>
                      )}
                      <button type="button" onClick={() => setCbPull({loading:false, products:null})} style={{marginLeft:'auto', border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:12, fontWeight:600}}>完成 ✓</button>
                    </div>
                    <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))', gap:10}}>
                      {cbPull.products.map((p, i) => { const saved = cbIsImgSaved(p); return (
                        <div key={i} title={p.title || ''}
                          style={{position:'relative', border: saved ? '2px solid #16a34a' : '1px solid var(--line)', borderRadius:8, overflow:'hidden', textAlign:'center', background: saved ? '#f0fdf4' : 'white'}}>
                          {saved && <div style={{position:'absolute', top:6, right:6, width:22, height:22, borderRadius:'50%', background:'#16a34a', color:'white', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, zIndex:1}}>✓</div>}
                          {p.image_url ? <img src={p.image_url} alt="" onClick={() => wsOpenImg(p.image_url)} title="点击看大图(新标签打开)" style={{width:'100%', height:90, objectFit:'cover', cursor:'zoom-in'}} /> : <div style={{height:90, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--ink-4)', fontSize:10}}>无图</div>}
                          <div style={{fontSize:10, color:'var(--ink-2)', padding:'4px 6px 0', lineHeight:1.2, maxHeight:26, overflow:'hidden'}}>{p.title || ''}</div>
                          <button type="button" onClick={() => cbPickImg(p)} disabled={!p.image_url}
                            style={{width:'100%', marginTop:4, padding:'6px 0', border:'none', borderTop:'1px solid var(--line)', cursor: p.image_url ? 'pointer' : 'not-allowed', fontSize:11, fontWeight:700,
                              background: saved ? '#dcfce7' : '#e0f2fe', color: saved ? '#15803d' : '#0369a1'}}>
                            {saved ? '✓ 已加入(点取消)' : '➕ 加入证据'}
                          </button>
                        </div>
                      ); })}
                    ...
                    {(evidence || []).filter(a => a.from_order).length > 0 && (
                      <div style={{marginTop:10, fontSize:12, color:'#15803d', fontWeight:700}}>✅ 已加入 {(evidence || []).filter(a => a.from_order).length} 张灯图 → 显示在下方「证据材料」· 点「创建拒付/保存修改」即存入</div>
                    )}
                  </>
                )}
              </div>
...
const TransferToPoModal = ({ order, user, onClose, onTransferred, toast }) => {
  ...
  const transfer = async () => {
    ...
    if (!poUserId && !poUserName.trim()) {
      if (!await wsConfirm('未指定跟单负责人,确定全跟单部都能看到吗?')) return;
    }
    setSending(true);
    try {
      const userName = user.name + (user.alias ? ' ' + user.alias : '');
      ...
      <div style={{padding:'12px 16px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, flexWrap:'wrap'}}>
          <div style={{flex:1, minWidth:0}}>
            ...
            {/* 🆕 fix141: 折叠态铺付款凭证/附件缩略图,点击看大图 */}
            <AttachThumbs files={attachments} size={48} max={6} onPreview={window.__setPreviewImg} />
          </div>
        ...
        {/* 🆕 fix143: 流程步骤条 录入→付款→转跟单 */}
        {(() => {
          const steps = [
            { id:'entry', label:'录入', icon:'📝', who: order.created_by_name, when: fmtStepDate(order.created_at), done: !!order.created_by_name && !!order.created_at, color:'#2563eb' },
            { id:'paid',  label:'付款', icon:'💰', who: null, when: fmtStepDate(order.paid_at), done: !!order.paid_at || ['paid','dispatched'].includes(order.status), color:'#d97706' },
            { id:'po',    label:'转跟单', icon:'📤', who: order.transferred_to_name, when: fmtStepDate(order.transferred_at), done: !!order.transferred_to_name && !!order.transferred_at, color:'#16a34a' },
          ];
          return <div style={{marginTop:10, paddingTop:10, borderTop:'1px dashed var(--line)'}}><ProcessStepper steps={steps} /></div>;
        })()}
    ...
    if (!isEdit) {
      try {
        ...
        if (existing && existing.length > 0) {
          if (!await wsConfirm(`⚠ 警告:订单编号 "${orderNo}" 已存在!\n\n继续保存将创建重复编号(可能导致管理混乱)。\n\n建议:点击"🔄 重新扫描+1"获取唯一编号。\n\n仍要继续保存吗?`)) {
            return;
          }
        }
      } catch {}
      ...
      <div style={{padding:'12px 16px'}}>
        <div style={{display:'flex', justifyContent:'space-between', gap:10, flexWrap:'wrap'}}>
          <div style={{flex:1, minWidth:0}}>
            ...
            {/* 🆕 fix141: 折叠态铺缩略图(参考图 + 实拍图),点击看大图,不必展开 */}
            <AttachThumbs files={[...refImgs, ...photos]} size={48} max={8} onPreview={window.__setPreviewImg} />
          </div>
          <div style={{display:'flex', gap:4, flexWrap:'wrap'}}>
...
const CustomInquiryEditor = ({ item, user, onClose, onSaved, toast }) => {
  ...
  const [cuPull, setCuPull] = useState({ loading: false, products: null });
  const cuDoPull = async () => {
    const on = (orderNo || '').trim();
    if (!on) { alert('请先填关联订单编号'); return; }
    setCuPull({ loading: true, products: null });
    try {
      const v = await wsFetchOrderProducts(on);
      setCuPull({ loading: false, products: (v && v.products) || [] });
      if (v) {
        if (!(customerEmail || '').trim() && v.email) setCustomerEmail(v.email);
        if (!(customerName || '').trim() && v.customerName) setCustomerName(v.customerName);
      }
    } catch (e) { setCuPull({ loading: false, products: [] }); }
  };
  const cuIsSaved = (p) => (refImages || []).some(a => (a.url || a.dataUrl) === p.image_url);
  const cuPick = (p) => {
    if (!p.image_url) return;
    if (cuIsSaved(p)) setRefImages((refImages || []).filter(a => (a.url || a.dataUrl) !== p.image_url));
    else setRefImages([...(refImages || []), { id: 'a_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7), url: p.image_url, label: '产品图', kind: 'image', mime: 'image/jpeg', name: (p.title || 'product') + '.jpg', from_order: true, uploaded_at: new Date().toISOString() }]);
  };
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12}}>
            ...
            <div style={{display:'flex', gap:6}}>
              <input value={orderNo} onChange={e => setOrderNo(e.target.value)} placeholder="关联订单编号"
                style={{flex:1, minWidth:0, padding:'7px 10px', border:'1px solid var(--line)', borderRadius:6, fontSize:13, fontFamily:'monospace'}} />
              <button type="button" onClick={cuDoPull} disabled={cuPull.loading} title="拉取订单:自动填客户,并可选产品图入参考图"
                style={{whiteSpace:'nowrap', padding:'7px 10px', border:'none', borderRadius:6, background:'#0071e3', color:'white', fontSize:12, fontWeight:600, cursor:'pointer', flexShrink:0}}>{cuPull.loading ? '拉取中…' : '🔄 拉取'}</button>
            </div>
          </div>
          {cuPull.products && (
            <div style={{marginBottom:12, padding:8, background:'#f8fafc', border:'1px solid var(--line)', borderRadius:8}}>
              {cuPull.products.length === 0 ? (
                <div style={{fontSize:11, color:'var(--ink-4)', display:'flex', justifyContent:'space-between'}}><span>没拉到该订单的产品(已尽量填客户)</span><button type="button" onClick={() => setCuPull({loading:false, products:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11}}>收起</button></div>
              ) : (
                <>
                  <div style={{fontSize:11, color:'var(--ink-3)', marginBottom:6, display:'flex', justifyContent:'space-between'}}><span>拉到 {cuPull.products.length} 个产品 · 点选存入参考图(再点取消)</span><button type="button" onClick={() => setCuPull({loading:false, products:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11, fontWeight:600}}>完成 ✓</button></div>
                  <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(90px,1fr))', gap:8}}>
                    {cuPull.products.map((p, i) => { const saved = cuIsSaved(p); return (
                      <div key={i} onClick={() => cuPick(p)} title={p.title || ''} style={{position:'relative', border: saved ? '2px solid #16a34a' : '1px solid var(--line)', borderRadius:8, padding:5, cursor:'pointer', textAlign:'center', background: saved ? '#f0fdf4' : 'white'}}>
                        {saved && <div style={{position:'absolute', top:-7, right:-7, width:20, height:20, borderRadius:'50%', background:'#16a34a', color:'white', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700}}>✓</div>}
                        {p.image_url ? <img src={p.image_url} alt="" style={{width:'100%', height:60, objectFit:'cover', borderRadius:5}} /> : <div style={{height:60, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--ink-4)', fontSize:10}}>无图</div>}
                        <div style={{fontSize:10, color:'var(--ink-2)', marginTop:4, lineHeight:1.2, maxHeight:24, overflow:'hidden'}}>{p.title || ''}</div>
                      </div>
                    ); })}
                  </div>
                </>
      ...
      <div style={{padding:'12px 16px'}}>
        <div style={{display:'flex', justifyContent:'space-between', gap:10, flexWrap:'wrap'}}>
          <div style={{flex:1, minWidth:0}}>
            ...
            {/* 🆕 fix141: 折叠态铺缩略图(销售展示图 + 实拍图),点击看大图 */}
            <AttachThumbs files={[...(item.sales_photos || []), ...(item.actual_photos || [])]} size={48} max={8} onPreview={window.__setPreviewImg} />
          </div>
          <div style={{display:'flex', gap:4, flexWrap:'wrap'}}>
...
const PhotoVerificationEditor = ({ item, user, onClose, onSaved, toast }) => {
  ...
  const [pvPull, setPvPull] = useState({ loading: false, products: null });
  const pvDoPull = async () => {
    const on = (orderRef || '').trim();
    if (!on) { alert('请先填订单编号'); return; }
    setPvPull({ loading: true, products: null });
    try {
      const v = await wsFetchOrderProducts(on);
      setPvPull({ loading: false, products: (v && v.products) || [] });
      if (v && !(productName || '').trim() && v.products && v.products[0] && v.products[0].title) setProductName(v.products[0].title);
    } catch (e) { setPvPull({ loading: false, products: [] }); }
  };
  const pvIsSaved = (p) => (salesPhotos || []).some(a => (a.url || a.dataUrl) === p.image_url);
  const pvPick = (p) => {
    if (!p.image_url) return;
    if (p.title) setProductName(p.title);
    if (pvIsSaved(p)) setSalesPhotos((salesPhotos || []).filter(a => (a.url || a.dataUrl) !== p.image_url));
    else setSalesPhotos([...(salesPhotos || []), { id: 'a_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7), url: p.image_url, label: '销售图(产品)', kind: 'image', mime: 'image/jpeg', name: (p.title || 'product') + '.jpg', from_order: true, uploaded_at: new Date().toISOString() }]);
  };
            ...
            <div>
              ...
              <div style={{display:'flex', gap:6}}>
                <input value={orderRef} onChange={e => setOrderRef(e.target.value)} placeholder="V99299" autoFocus
                  style={{flex:1, minWidth:0, padding:'7px 10px', border:'1px solid var(--line)', borderRadius:6, fontSize:13, fontFamily:'monospace'}} />
                <button type="button" onClick={pvDoPull} disabled={pvPull.loading} title="拉取订单:自动填产品名,并可选产品图入销售图"
                  style={{whiteSpace:'nowrap', padding:'7px 10px', border:'none', borderRadius:6, background:'#0071e3', color:'white', fontSize:12, fontWeight:600, cursor:'pointer', flexShrink:0}}>{pvPull.loading ? '拉取中…' : '🔄 拉取'}</button>
              </div>
              {pvPull.products && (
                <div style={{marginTop:8, padding:8, background:'#f8fafc', border:'1px solid var(--line)', borderRadius:8}}>
                  {pvPull.products.length === 0 ? (
                    <div style={{fontSize:11, color:'var(--ink-4)', display:'flex', justifyContent:'space-between'}}><span>没拉到该订单的产品</span><button type="button" onClick={() => setPvPull({loading:false, products:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11}}>收起</button></div>
                  ) : (
                    <>
                      <div style={{fontSize:11, color:'var(--ink-3)', marginBottom:6, display:'flex', justifyContent:'space-between'}}><span>拉到 {pvPull.products.length} 个 · 点选存入销售图(再点取消)</span><button type="button" onClick={() => setPvPull({loading:false, products:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11, fontWeight:600}}>完成 ✓</button></div>
                      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(90px,1fr))', gap:8}}>
                        {pvPull.products.map((p, i) => { const saved = pvIsSaved(p); return (
                          <div key={i} onClick={() => pvPick(p)} title={p.title || ''} style={{position:'relative', border: saved ? '2px solid #16a34a' : '1px solid var(--line)', borderRadius:8, padding:5, cursor:'pointer', textAlign:'center', background: saved ? '#f0fdf4' : 'white'}}>
                            {saved && <div style={{position:'absolute', top:-7, right:-7, width:20, height:20, borderRadius:'50%', background:'#16a34a', color:'white', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700}}>✓</div>}
                            {p.image_url ? <img src={p.image_url} alt="" style={{width:'100%', height:60, objectFit:'cover', borderRadius:5}} /> : <div style={{height:60, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--ink-4)', fontSize:10}}>无图</div>}
                            <div style={{fontSize:10, color:'var(--ink-2)', marginTop:4, lineHeight:1.2, maxHeight:24, overflow:'hidden'}}>{p.title || ''}</div>
                          </div>
...
const CustomerRepliesBoard = ({ replies, setReplies, user }) => {
  ...
  const removeReply = async (id) => {
    if (!await wsConfirm('确认删除这条留言?')) return;
