...
// 📦 版本日志 - 用户用来确认加载的是哪个版本
const APP_VERSION = '2026.05.25-fix20';
...
const VERSION_HISTORY = [
  { version: '2026.05.25-fix20', date: '2026.05.25', title: '📦 模块化重构 — 1.32MB 单文件拆成 11 个 JS 模块', changes: [
    { type:'refactor', text:'workspace.html 从 23k 行单文件拆成 21KB 壳 + 11 个独立 JS 文件,加载速度更快,维护更清晰' },
    { type:'refactor', text:'拆分原则:按功能模块归类 — 核心工具 / 客服跟进 / 看板回收站 / 设置财务 / 报价会议纪要 / 拒付线下单 / 实拍评价 / 工作主线 / 知识库跨部门 / 任务反馈 / 帮助主入口' },
    { type:'refactor', text:'部署方式不变 — 仍是纯静态 HTML + JS,无需打包步骤,继续走 GitHub Pages' },
    { type:'refactor', text:'Babel-standalone 顺序加载 11 个文件,顶层 const 经 env preset 转 var 自动注入全局,任意文件互引用' },
    { type:'perf', text:'单文件下载 1.32MB → 拆 11 文件按需缓存,改一个模块只刷该文件,CDN 命中率更高' },
  ]},
