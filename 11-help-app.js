...
const App = () => {
  ...
  useEffect(() => { window.__setPreviewImg = setPreviewImg; return () => { if (window.__setPreviewImg === setPreviewImg) window.__setPreviewImg = null; }; }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      const cloud = await loadCloudAccounts();
      if (cloud === null) {
      ...
      if (cloud.length === 0) {
        // 首次:把当前本地名单(内置 + 本地新增)播种到云端
        await seedCloudAccounts(employees);
        return;
      }
      // 云端为准;补上云端没有的内置账号;本机有而云端没有的(离线新增)推到云端
      const byId = new Map(cloud.map(e => [e.id, e]));
      INITIAL_EMPLOYEES.forEach(i => { if (!byId.has(i.id)) { byId.set(i.id, i); saveCloudAccount(i); } });
      employees.forEach(e => { if (!byId.has(e.id)) { byId.set(e.id, e); saveCloudAccount(e); } });
  ...
  useEffect(() => {
    try { localStorage.setItem(STORE.k('cs_records'), JSON.stringify(records)); }
    catch(e) { STORE.set('cs_records', slimRecordsForCache(records)); }
  }, [records]);
  ...
  const [navFocus, setNavFocus] = useState(null);  // { tab, id, sub, t } | null
  const goFocus = useCallback((tab, id, sub) => {
    if (!tab) return;
    setActiveTab(tab);
    setNavFocus(id ? { tab, id, sub: sub || null, t: Date.now() } : null);
  }, []);
  const focusFor = (tab) => (navFocus && navFocus.tab === tab ? navFocus.id : null);
  ...
  const cdmMaxCreatedRef = useRef(0);
  const cdmCatchupTimer = useRef(null);
  ...
  const cdmLight = (m) => ({ ...m, attachments: [], thread: [] });
  const cdmBumpWatermark = (rows) => {
    let mx = cdmMaxCreatedRef.current;
    (rows || []).forEach(r => { const v = +(r.created_at_ms || 0); if (v > mx) mx = v; });
    cdmMaxCreatedRef.current = mx;
  };

  const loadCdmMessages = async () => {
    if (cdmLoadingRef.current) return;  // 防止重复请求
    ...
    try {
      ...
      let { data, error } = await client
        ...
        .select(CDM_LIST_COLS)
        .gte('created_at_ms', cutoffMs)
        .order('created_at_ms', { ascending: false })
        .limit(500);
      // 🆕 fix137: 轻量列里若有列名和本表对不上(如某列不存在),自动回退 select('*'),保证一定能加载
      if (error) {
        console.warn('[CDM] 轻量列查询失败,回退 select(*):', error.message || error);
        const fb = await client.from('cross_dept_messages').select('*').gte('created_at_ms', cutoffMs).order('created_at_ms', { ascending: false }).limit(500);
        data = fb.data; error = fb.error;
      }
      if (error) throw error;
      cdmBumpWatermark(data);  // 🆕 fix140
      // 🆕 fix137: 列表项补 attachments/thread 空数组(轻量列不含它们),避免消费方读 .length/.filter 报错导致整页崩
      setCdmMessages((data || []).map(m => ({ ...m, attachments: Array.isArray(m.attachments) ? m.attachments : [], thread: Array.isArray(m.thread) ? m.thread : [] })));
  ...
  const cdmIncrementalFetch = async () => {
    const client = getCdmClient();
    if (!client) return;
    const since = cdmMaxCreatedRef.current;
    if (!since) return loadCdmMessages();  // 还没水位 → 退化为一次初拉
    try {
      ...
      setCdmMessages(prev => {
        ...
        const fresh = data.filter(m => !ids.has(m.id)).map(cdmLight);
        return fresh.length ? [...fresh, ...prev] : prev;
      });
    } catch (e) {}
  };
  const scheduleCdmCatchup = () => {
    clearTimeout(cdmCatchupTimer.current);
    cdmCatchupTimer.current = setTimeout(() => cdmIncrementalFetch(), 1500);  // 防抖 1.5s,合并突发
  };
      ...
      channel = client
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'cross_dept_messages' }, (payload) => {
          ...
          if (ev === 'INSERT' && payload.new) {
            const row = cdmLight(payload.new);
            cdmBumpWatermark([payload.new]);
            setCdmMessages(prev => prev.some(m => m.id === row.id) ? prev : [row, ...prev]);
            // 新消息且发给客服部 → 桌面通知
            ...
            const row = cdmLight(payload.new);  // 仅合并轻量字段(状态/read_by/指派等),大列恒为 []
            setCdmMessages(prev => {
              let hit = false;
              const next = prev.map(m => { if (m.id === row.id) { hit = true; return { ...m, ...row }; } return m; });
              return hit ? next : prev;  // 不在当前窗口的更新 → 忽略(兜底补拉会捞新行)
            });
          } else if (ev === 'DELETE' && payload.old) {
            setCdmMessages(prev => prev.filter(m => m.id !== payload.old.id));
          } else {
  ...
  const isAdmin = (user?.role === 'admin' || user?.role === 'super_admin');
  const DEFAULT_TOP_KEYS = isAdmin
    ? ['dashboard', 'cs', 'chargebacks', 'offline_orders', 'custom_photo', 'events']
    : ['cs', 'chargebacks', 'offline_orders', 'custom_photo', 'events', 'reviews'];
  ...
  const allTabs = useMemo(() => {
    ...
    const tabs = [
      ...
      { key:'kpi_scorer', label:'📋 绩效考核', icon:'📋', group:'resources' },
      ...
      { key:'influencers', label:'✨ 影响者合作', icon:'✨', group:'collab' },
      { key:'workflows', label:'📋 工作流/SOP', icon:'📋', group:'resources' },
      ...
      ...(isAdmin ? [
        ...
        { key:'admin_overview',   label:'📊 绩效总览', icon:'📊', group:'admin' },
        { key:'dashboard',        label:'📈 数据看板', icon:'📈', group:'admin' },
      ] : [
        { key:'dashboard', label:'📈 数据看板', icon:'📈', group:'admin' },
        { key:'trash',     label:'🗑 回收站',   icon:'🗑', badge: stats?.trashCount, badgeColor:'#86868b', group:'admin' },
    ...
    if (user && user.role === 'hr') {
      const hrAllow = ['admin_overview', 'kpi_scorer', 'dashboard'];
      return tabs.filter(t => hrAllow.includes(t.key));
    }
    return tabs;
  ...
  const onLogout = async () => {
    if (!await wsConfirm('确认退出登录？')) return;
    setUser(null);
  ...
  // 🆕 切换账号 - 主管/老板免密查看模式
  const switchAccount = async (targetEmployee) => {
    ...
    // 普通员工切换:需要密码
    const password = await wsPrompt(`切换到 ${targetEmployee.name}${targetEmployee.alias ? ' ' + targetEmployee.alias : ''} 的账号\n\n请输入该账号的密码：`);
  ...
  return (
    ...
    <div className="min-h-screen">
      {/* 🆕 fix141: App 级单例图片预览弹窗 */}
      <ImgPreviewModal img={previewImg} onClose={() => setPreviewImg(null)} />
      ...
      <GlobalSearch 
        ...
        onJumpToRecord={(target) => goFocus(target.tab, target.recordId, target.subTab)}
      />
      ...
      <div style={{display:'flex', alignItems:'flex-start', minHeight:'calc(100vh - 56px)'}}>
        ...
        {user && cloudOn && activeTab !== 'chargebacks' && (
          <ChargebackReminderBanner user={user} employees={employees} onJumpTo={(cb) => goFocus('chargebacks', cb && cb.id)} />
        )}
        {activeTab === 'cs' && (
          <>
            ...
            <CSModule user={user} employees={employees} records={records} setRecords={setRecords} toast={toast} cloudOn={cloudOn} focusId={focusFor('cs')} onOpenQuoteTab={() => setActiveTab('quote')} />
          </>
        )}
        {activeTab === 'chargebacks' && (
          <ChargebacksModule user={user} employees={employees} toast={toast} focusId={focusFor('chargebacks')} />
        )}
        ...
        {activeTab === 'influencers' && (
          <InfluencerModule user={user} employees={employees} toast={toast} />
        )}
        {activeTab === 'workflows' && (
          <WorkflowModule goTab={setActiveTab} />
        )}
        {activeTab === 'events' && (
          <EventsModule user={user} employees={employees} records={records} toast={toast} cloudOn={cloudOn}
            focusId={focusFor('events')} focusSub={navFocus && navFocus.tab === 'events' ? navFocus.sub : null} />
        )}
        {activeTab === 'reviews' && (
        ...
        {/* 🆕 fix58: 运费精算器(独立 iframe) */}
        {activeTab === 'freight_calc' && (
          <FreightCalcModule user={user} toast={toast} />
        )}
        {activeTab === 'kpi_scorer' && (
          <KpiScorerModule user={user} toast={toast} />
        )}
        {/* 🆕 fix69: 快递发票制作 */}
        ...
        {activeTab === 'admin_overview' && (user.role === 'admin' || user.role === 'super_admin' || user.role === 'hr') && (
          <>
            <WorkSnapshotPanel user={user} employees={employees} records={records}
...
// 📦 版本日志 - 用户用来确认加载的是哪个版本
const APP_VERSION = '2026.06.05-fix152';
