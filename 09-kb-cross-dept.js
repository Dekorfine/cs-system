...
const SuppliersManagement = ({ toast, user }) => {
  ...
  const handleDelete = async (s) => {
    if (s.total_aftersales > 0) {
      if (!await wsConfirm(`供应商 "${s.name}" 有 ${s.total_aftersales} 单售后历史，确定要删除吗？\n\n建议改为停用而非删除。`)) return;
    } else {
      if (!await wsConfirm(`确认删除供应商 "${s.name}"？`)) return;
    }
...
const EmailTemplatesModule = ({ user, toast }) => {
  ...
  const handleDelete = async (id) => {
    if (!await wsConfirm('确定删除这个模板?(预置模板不能删,只能改自定义)')) return;
...
const FreightCalcModule = ({ user, toast }) => {
  ...
  return (
    <div className="paper rounded-2xl" style={{padding:'14px 14px 14px', overflow:'hidden'}}>
      <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:10, padding:'0 4px', flexWrap:'wrap'}}>
        <div className="font-display" style={{fontSize:20, fontWeight:600, letterSpacing:'-.022em', flex:1, minWidth:180}}>
          🚚 运费精算器
          <span style={{fontSize:12, fontWeight:400, color:'var(--ink-3)', marginLeft:8}}>
...
// ════════════════════════════════════════════════════════════════════
const KpiScorerModule = ({ user, toast }) => {
  const [loadStatus, setLoadStatus] = useState('loading');
  const [iframeUrl, setIframeUrl] = useState(() => `kpi-scorer.html?t=${Date.now().toString(36)}`);
  useEffect(() => {
    const timer = setTimeout(() => setLoadStatus(s => s === 'loading' ? 'timeout' : s), 8000);
    return () => clearTimeout(timer);
  }, [iframeUrl]);
  const reload = () => { setLoadStatus('loading'); setIframeUrl(`kpi-scorer.html?t=${Date.now().toString(36)}`); };
  return (
    <div className="paper rounded-2xl" style={{padding:'14px 14px 14px', overflow:'hidden'}}>
      <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:10, padding:'0 4px', flexWrap:'wrap'}}>
        <div className="font-display" style={{fontSize:20, fontWeight:600, letterSpacing:'-.022em', flex:1, minWidth:180}}>
          📋 绩效考核打分器
          <span style={{fontSize:12, fontWeight:400, color:'var(--ink-3)', marginLeft:8}}>业绩75 + 能力25 + 奖惩 · 自评/主管评分 · 每项依据+图片 · 日报/月报</span>
        </div>
      ...
      {loadStatus === 'timeout' && (
        <div style={{background:'#fef3c7', border:'1px solid #f59e0b', borderRadius:10, padding:'12px 14px', marginBottom:10, fontSize:13, color:'#92400e', lineHeight:1.6}}>
          ⚠ <strong>绩效打分器加载超时</strong>(8 秒内未响应)
          <div style={{marginTop:6, fontSize:12}}>可能原因:kpi-scorer.html 没有跟 index.html 一起部署。请确认 GitHub Pages 仓库根目录有 kpi-scorer.html。</div>
          <button onClick={reload} style={{marginTop:8, padding:'5px 12px', fontSize:12, background:'#f59e0b', color:'white', border:'none', borderRadius:6, cursor:'pointer', fontFamily:'inherit'}}>🔄 再试一次</button>
        </div>
      )}
      <iframe key={iframeUrl} src={iframeUrl} title="绩效考核打分器" onLoad={() => setLoadStatus('loaded')}
...
const WORKFLOWS = [
  {
    id:'chargeback', icon:'🚨', title:'拒付处理', color:'#dc2626', bg:'#fef2f2', linkTab:'chargebacks', linkLabel:'去拒付模块',
    summary:'收到拒付通知 → 记时限 → 判断类型 → 取证 → 时限内应诉 → 跟进结果',
    warning:'⏰ 拒付有应诉时限(常见 7–10 天,以支付平台通知为准)。逾期 = 自动判输,务必当天处理、第一时间告知主管。',
    steps:[
      { n:1, title:'确认拒付通知 + 记录时限', desc:'收到拒付(Chargeback / Dispute)通知后立刻处理,先把关键信息记下来。',
        checklist:['记录拒付金额 + 原因代码(Reason Code)','查清应诉截止日期(Respond by)','在「客服跟进」把该订单标记为「拒付」分类','立即告知主管'] },
      { n:2, title:'判断拒付类型(决定取证方向)', desc:'不同类型要准备不同证据,先判断属于哪一种:',
        branches:[
          { cond:'未收到货 (Product Not Received)', action:'取证:物流单号 + 全程轨迹 + 签收/妥投证明 + 投递定位' },
          { cond:'货不对板 / 有损 (Not as Described)', action:'取证:产品页描述与图片截图 + 与客户沟通记录 + 实拍对比;若确属质量问题,考虑直接退款止损' },
          { cond:'未授权 / 欺诈 (Fraudulent)', action:'取证:AVS/CVV 匹配、账单地址=收货地址、下单 IP/设备、客户下单及收货确认记录' },
          { cond:'重复扣款 (Duplicate)', action:'核对是否真重复:是 → 主动退一笔并举证;否 → 举证两笔为不同订单' },
          { cond:'已取消未退款 (Credit Not Processed)', action:'查退款记录:已退 → 举证退款凭证;漏退 → 立即退款并说明' },
        ] },
      { n:3, title:'收集证据材料(通用清单)', desc:'无论哪种类型,这些都尽量备齐:',
        checklist:['订单详情:下单时间 / 邮箱 / 下单 IP','物流全程轨迹 + 签收/妥投证明','与客户的全部沟通记录(邮件/消息)','产品页与实物一致的证明(描述/图片)','退款/物流政策客户已知悉或同意的截图'] },
      { n:4, title:'在支付平台 / Shopify 提交应诉', desc:'在截止日前提交。材料齐全、按时间线简明陈述事实,附上上一步的证据。',
        note:'具体提交入口按公司用的支付平台 [按公司规定填:如 Shopify Payments / PayPal / Stripe]。拿不准就先问主管。' },
      { n:5, title:'跟进结果 + 复盘', desc:'记录应诉结果(赢/输)。输了分析原因;同一类拒付高发时,把原因反馈给运营/产品改进(如物流时效、产品描述)。',
        checklist:['在系统记录最终结果','输了 → 记录原因','高发原因 → 反馈运营/产品'] },
    ],
  },
  {
    id:'express_invoice', icon:'📦', title:'快递发票(昌晖 / 云鼎)', color:'#c2410c', bg:'#fff7ed', linkTab:'express_invoice', linkLabel:'去快递发票模块',
    summary:'选渠道 → 从对应表导入 / 单查 → 建票 → 逐张核对 → 导出给快递',
    warning:'昌晖和云鼎是两套不同流程。先看货实际走哪个快递,再选渠道——选错了表和模板都不对。',
    steps:[
      { n:1, title:'选渠道', desc:'在「快递发票」工具顶部「快递公司」下拉,选这批货实际走的快递。',
        branches:[
          { cond:'昌晖', action:'走「北简表」导入。客户订单号列填客户参考号(XM8T,快递核对用),我方单号写进运单备注' },
          { cond:'云鼎(YL-洛杉矶直送超大件卡派)', action:'走「交接清单」导入。每个订单生成一个独立 Excel,文件名=订单号 1票N件' },
        ] },
      { n:2, title:'导入建票 / 单票查询', desc:'两种方式:批量上传,或单个查。',
        branches:[
          { cond:'批量 · 昌晖', action:'上传北简表(已手填「订单编号」)→ 自动查库建票' },
          { cond:'批量 · 云鼎', action:'上传交接清单 → 按「单号」建票;DF17280/DF17392 自动合并成一单、K114157-1 子单自动处理' },
          { cond:'单票', action:'用顶部「🔎 快速查单」输订单号 → 弹窗核对细节 + 图片 → 加入发票篮' },
        ] },
      { n:3, title:'逐张核对(最关键)', desc:'建好票后,在发票篮里逐张检查:',
        checklist:[
          '收件人信息完整:名称/电话/邮编/城市/省州/地址1(云鼎不超 35 字符)/邮箱',
          '只保留真实灯具(OrderArmor 等保险已自动剔除)',
          '申报灯型对(壁灯 Wall Lamp / 吊灯 Pendant / 枝形吊灯 Chandelier… 按真实灯型,逐行可改)',
          '云鼎:选中数量 = 交接清单「件数」;只发部分的(如 K..-1 发6件)调数量或取消勾选',
          '同款多 SKU 已合并成一行、数量累加',
        ] },
      { n:4, title:'导出', desc:'核对无误(篮子显示「必齐」)后导出。',
        branches:[
          { cond:'昌晖', action:'导出一个大 Excel(每单一行)' },
          { cond:'云鼎', action:'每单一个 Excel(文件名=订单号 1票N件),多单自动打包 ZIP,产品图自动嵌入框内' },
        ] },
      { n:5, title:'交付快递公司', desc:'把导出的发票文件发给对应快递公司(昌晖 / 云鼎)。云鼎产品图嵌不进的(个别跨域),导出后检查一下框内有没有图。' },
    ],
  },
  {
    id:'refund', icon:'💰', title:'退款', color:'#15803d', bg:'#f0fdf4', linkTab:'cs', linkLabel:'去客服跟进',
    summary:'核实原因 → 判断全退/部分退/改补件 → 走审批 → 执行 → 通知并记录',
    warning:'金额、审批门槛、原路退回都按公司财务规定来 [按公司规定填具体门槛]。拿不准先问主管,不要自行决定大额退款。',
    steps:[
      { n:1, title:'核实退款原因 + 是否符合政策', desc:'先确认订单状态、退款原因(质量/物流/客户主观),以及是否在退款窗口内、是否符合公司退款政策。',
        checklist:['查订单当前状态(已发/未发/已签收)','明确退款原因','确认是否在退款时效内 [按公司规定]'] },
      { n:2, title:'判断退款方式', desc:'根据情况选处理方式:',
        branches:[
          { cond:'全额退 (Full)', action:'常见:未发货取消 / 重大质量问题 / 物流确认丢件' },
          { cond:'部分退 (Partial)', action:'轻微瑕疵客户愿留用 / 补偿性折让 / 运费差额' },
          { cond:'不退,改补件或换货', action:'缺件或可修复 → 走「补件」流程,不走退款' },
        ] },
      { n:3, title:'走审批', desc:'按金额走审批流程。',
        note:'[按公司规定:小额客服可自处理 / 超过 X 金额需主管审批]。审批通过后在系统记录审批人。' },
      { n:4, title:'执行退款', desc:'在 Shopify / 支付平台原路退回到客户原支付方式。',
        checklist:['原路退回(不要换渠道转账)','记录退款单号 / 金额 / 日期'] },
      { n:5, title:'通知客户 + 记录归档', desc:'告知客户退款已发起,并在系统留痕。',
        checklist:['邮件/消息通知客户(到账约 3–5 个工作日,以银行为准)','在「客服跟进」标记「退款」分类','备注金额 / 原因 / 审批人'] },
    ],
  },
];

const WorkflowModule = ({ goTab }) => {
  const [sel, setSel] = useState(null);
  if (sel) {
    const wf = WORKFLOWS.find(w => w.id === sel);
    if (!wf) { setSel(null); return null; }
    return (
      <div className="max-w-4xl mx-auto">
        <button onClick={()=>setSel(null)} className="text-sm text-stone-500 hover:text-stone-700 mb-3">← 返回工作流列表</button>
        <div className="rounded-xl p-4 mb-4" style={{background:wf.bg}}>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-2xl">{wf.icon}</span>
            <h2 className="text-xl font-bold" style={{color:wf.color}}>{wf.title}</h2>
            {goTab && wf.linkTab && <button onClick={()=>goTab(wf.linkTab)} className="ml-auto text-sm px-3 py-1.5 rounded-lg bg-white border border-stone-200 hover:border-stone-300">👉 {wf.linkLabel}</button>}
          </div>
          <div className="text-sm text-stone-600 mt-1">{wf.summary}</div>
          {wf.warning && <div className="mt-3 text-sm bg-white/70 border-l-4 rounded px-3 py-2" style={{borderColor:wf.color, color:'#7c2d12'}}>{wf.warning}</div>}
        </div>
        {wf.steps.map(step => (
          <div key={step.n} className="border border-stone-200 rounded-xl p-4 mb-3 bg-white">
            <div className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-full text-white flex items-center justify-center text-sm font-bold flex-shrink-0" style={{background:wf.color}}>{step.n}</div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-stone-800">{step.title}</div>
                {step.desc && <div className="text-sm text-stone-600 mt-1 leading-relaxed">{step.desc}</div>}
                {step.checklist && <ul className="mt-2 space-y-1">{step.checklist.map((c,i)=>(<li key={i} className="text-sm text-stone-700 flex gap-2"><span className="text-stone-300">☐</span><span>{c}</span></li>))}</ul>}
                {step.branches && <div className="mt-2 space-y-1.5">{step.branches.map((b,i)=>(<div key={i} className="bg-amber-50 border-l-2 border-amber-400 px-3 py-1.5 rounded text-sm text-stone-700"><b className="text-amber-900">{b.cond}</b> → {b.action}</div>))}</div>}
                {step.note && <div className="mt-2 text-xs text-stone-500 bg-stone-50 rounded px-2.5 py-1.5">💡 {step.note}</div>}
              </div>
            </div>
          </div>
        ))}
        <div className="text-xs text-stone-400 text-center mt-4 mb-2">标 [按公司规定] 的地方按你们实际政策填 · 流程有变化告诉我随时更新</div>
      </div>
    );
  }
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-1 flex-wrap">
        <h2 className="text-xl font-bold text-stone-800">📋 工作流 / SOP</h2>
        <span className="text-xs text-stone-400">新人友好 · 按步骤走的备用指引</span>
      </div>
      <div className="text-sm text-stone-500 mb-4">点开任一流程,按编号步骤操作。含判断分叉、核对清单,可直接跳转到对应工具。</div>
      <div className="grid gap-3" style={{gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))'}}>
        {WORKFLOWS.map(wf => (
          <button key={wf.id} onClick={()=>setSel(wf.id)} className="text-left rounded-xl p-4 border border-stone-200 hover:shadow-md transition bg-white">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xl">{wf.icon}</span>
              <span className="font-bold" style={{color:wf.color}}>{wf.title}</span>
              <span className="ml-auto text-xs text-stone-400">{wf.steps.length} 步</span>
            </div>
            <div className="text-sm text-stone-600 leading-relaxed">{wf.summary}</div>
          </button>
        ))}
      </div>
      <div className="text-xs text-stone-400 mt-5">更多流程(定制全流程 / 跨部门找对人 / 售后分诊…)可按需继续加。</div>
    </div>
  );
};
...
const INF_PLATFORMS = [
  ...
  { v:'pinterest', label:'Pinterest', emoji:'📌' },
  { v:'xiaohongshu', label:'小红书', emoji:'📕' },
  { v:'facebook', label:'Facebook', emoji:'👥' },
  { v:'other', label:'其他', emoji:'🌐' },
];
const INF_STATUSES = [
  { v:'contacted', label:'接触中', emoji:'✉️', color:'#78716c', bg:'#f5f5f4' },
  { v:'agreed', label:'已同意', emoji:'🤝', color:'#1d4ed8', bg:'#dbeafe' },
  { v:'sample_sent', label:'已寄样', emoji:'📦', color:'#c2410c', bg:'#ffedd5' },
  { v:'content_received', label:'收到内容', emoji:'📸', color:'#7c3aed', bg:'#ede9fe' },
  { v:'posted', label:'已发布', emoji:'🚀', color:'#0891b2', bg:'#cffafe' },
  { v:'completed', label:'已完成', emoji:'✅', color:'#15803d', bg:'#dcfce7' },
  { v:'declined', label:'已拒绝', emoji:'❌', color:'#dc2626', bg:'#fee2e2' },
];
const INF_REGIONS = ['US','UAE','AU','SA','FR','UK','other'];
const INF_COLLAB = [ {v:'gifted',label:'🎁 Gifted 寄样'}, {v:'paid',label:'💵 Paid 付费'}, {v:'affiliate',label:'🔗 Affiliate 联盟'} ];
const INF_RATINGS = ['S','A','B','C'];
const infPlat = v => INF_PLATFORMS.find(p=>p.v===v) || { v, label:v, emoji:'🌐' };
const infStat = v => INF_STATUSES.find(s=>s.v===v) || INF_STATUSES[0];
const infSafeArr = v => Array.isArray(v) ? v : (typeof v==='string' && v.trim().startsWith('[') ? (()=>{try{return JSON.parse(v);}catch(e){return [];}})() : []);
const infSafeObj = v => (v && typeof v==='object' && !Array.isArray(v)) ? v : (typeof v==='string' && v.trim().startsWith('{') ? (()=>{try{return JSON.parse(v);}catch(e){return {};}})() : {});
const INF_INP = 'px-3 py-2 text-sm border border-stone-300 rounded-lg w-full focus:border-pink-400 outline-none';
const InfL = ({ label, children, full }) => (
...
const InfCard = ({ b, onOpen, onStatus, onDel, compact }) => {
  const st = infStat(b.status);
  const emojis = (b.platforms||[]).map(p=>infPlat(p).emoji).join(' ');
  const spend = (b.sampleCost||0)+(b.shippingCost||0)+(b.fee||0);
  const m = b.metrics || {};
  return (
    <div className="border border-stone-200 rounded-xl bg-white p-3 hover:shadow-md transition">
      <div className="flex items-start gap-2">
        {b.productImage ? <img src={b.productImage} alt="" className="w-12 h-12 rounded object-cover flex-shrink-0 border border-stone-100" /> : <div className="w-12 h-12 rounded bg-stone-100 flex items-center justify-center flex-shrink-0 text-lg">💡</div>}
        <div className="min-w-0 flex-1">
          <div className="font-semibold text-stone-800 truncate cursor-pointer hover:text-pink-600" onClick={onOpen}>{emojis} {b.influencerName}</div>
          <div className="text-xs text-stone-500 truncate">{b.handle||''}{b.followerCount?(' · '+Number(b.followerCount).toLocaleString()+' 粉'):''}{b.region?(' · '+b.region):''}</div>
          {b.productName && <div className="text-xs text-stone-600 truncate mt-0.5">📦 {b.productName}</div>}
        </div>
      </div>
      <div className="flex items-center gap-2 mt-2 flex-wrap">
        <select value={b.status} onChange={e=>onStatus(e.target.value)} className="text-xs px-1.5 py-1 rounded border outline-none" style={{background:st.bg, color:st.color, borderColor:st.color+'55'}}>
          {INF_STATUSES.map(s=><option key={s.v} value={s.v}>{s.emoji} {s.label}</option>)}
        </select>
        {spend>0 && <span className="text-xs text-stone-400">¥{spend.toLocaleString()}</span>}
        {b.discountCode && <span className="text-xs px-1.5 py-0.5 rounded bg-amber-50 text-amber-700">{b.discountCode}</span>}
        {!compact && <span className="flex-1"></span>}
        {!compact && <button onClick={onOpen} className="text-xs text-blue-600 hover:underline">编辑</button>}
        {!compact && onDel && <button onClick={onDel} className="text-xs text-red-400 hover:underline">删</button>}
      </div>
      {!compact && (m.views || m.likes || m.sales || m.clicks) ? <div className="text-xs text-stone-400 mt-1.5 flex gap-3">{m.views?<span>👁 {Number(m.views).toLocaleString()}</span>:null}{m.likes?<span>❤️ {Number(m.likes).toLocaleString()}</span>:null}{m.clicks?<span>🔗 {m.clicks}</span>:null}{m.sales?<span>💰 {m.sales} 单</span>:null}</div> : null}
      {compact && b.deadline ? <div className="text-xs text-stone-400 mt-1">📅 {b.deadline}</div> : null}
    </div>
  );
};

const InfForm = ({ initial, onCancel, onSave, saving, toast }) => {
  const [f, setF] = useState({ ...initial });
  const up = (k,v) => setF(p=>({ ...p, [k]:v }));
  const togglePlat = v => { const cur=f.platforms||[]; up('platforms', cur.includes(v)?cur.filter(x=>x!==v):[...cur,v]); };
  const upMetric = (k,v) => up('metrics', { ...(f.metrics||{}), [k]: Number(v)||0 });
  const m = f.metrics || {};
  const FIN = 'w-full text-sm py-1.5 px-2 rounded-md border border-stone-300 focus:border-pink-400 outline-none';
  const Fld = ({ label, span, children }) => (
    <div className={span===4?'col-span-4':span===3?'col-span-3':span===2?'col-span-2':''}>
      <label className="text-[11px] font-medium text-stone-600 mb-0.5 block">{label}</label>
      {children}
    </div>
  );
  const briefSheet = () => {
    const plats = (f.platforms||[]).map(p=>{const d=INF_PLATFORMS.find(x=>x.v===p);return d?d.label:p;}).join(' + ');
    const txt = [
      `✨ VAKKERLIGHT × ${f.influencerName||''} — Collaboration Brief`, '',
      `Product: ${f.productName||''}${f.productSku?` (${f.productSku})`:''}`,
      plats?`Platform: ${plats}`:'', f.region?`Region: ${f.region}`:'',
      `Collab Type: ${f.collabType||'gifted'}`, '',
      (f.briefPoints||[]).length?'═══ Brief Points ═══':'', ...((f.briefPoints||[]).map(b=>`• ${b}`)),
      '', '═══ Posting ═══',
      f.postingRequirements?`- ${f.postingRequirements}`:'', f.deadline?`- Post by ${f.deadline}`:'',
      f.hashtags?`- Hashtags: ${f.hashtags}`:'', f.discountCode?`- Discount code: ${f.discountCode}`:'',
      f.dosDonts?`\n═══ Do's & Don'ts ═══\n${f.dosDonts}`:'',
    ].filter(x=>x!=='').join('\n');
    try { navigator.clipboard.writeText(txt); toast && toast('✓ Brief Sheet 已复制,可直接粘给博主'); }
    catch(e){ window.prompt('复制 Brief Sheet:', txt); }
  };
  return (
    <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 p-4 overflow-y-auto" onClick={onCancel}>
      <div className="bg-white rounded-2xl w-[92vw] max-w-[1200px] my-4 p-5 shadow-2xl" style={{maxHeight:'92vh',overflow:'auto'}} onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3 sticky top-0 bg-white pb-2 z-10">
          <h3 className="text-lg font-bold text-stone-800">{f.id?'编辑合作':'✨ 新建影响者合作'}</h3>
          <button onClick={onCancel} className="text-stone-400 text-2xl leading-none hover:text-stone-600">×</button>
        </div>

        <fieldset className="border border-stone-200 rounded-lg p-3 mb-3">
          <legend className="text-xs font-bold text-stone-700 px-1">👤 基本信息</legend>
          <div className="grid grid-cols-4 gap-2">
            <Fld label="博主名 *"><input className={FIN} value={f.influencerName||''} onChange={e=>up('influencerName',e.target.value)} placeholder="Lana Smith" /></Fld>
            <Fld label="Handle"><input className={FIN} value={f.handle||''} onChange={e=>up('handle',e.target.value)} placeholder="@lana_decor" /></Fld>
            <Fld label="粉丝数"><input type="number" className={FIN} value={f.followerCount||''} onChange={e=>up('followerCount',e.target.value)} /></Fld>
            <Fld label="区域"><select className={FIN} value={f.region||''} onChange={e=>up('region',e.target.value)}><option value="">—</option>{INF_REGIONS.map(r=><option key={r} value={r}>{r}</option>)}</select></Fld>
            <Fld label="主页 URL" span={2}><input className={FIN} value={f.profileUrl||''} onChange={e=>up('profileUrl',e.target.value)} placeholder="https://instagram.com/..." /></Fld>
            <Fld label="领域 niche"><input className={FIN} value={f.niche||''} onChange={e=>up('niche',e.target.value)} placeholder="home decor" /></Fld>
            <Fld label="联系方式"><input className={FIN} value={f.contact||''} onChange={e=>up('contact',e.target.value)} placeholder="email / WhatsApp" /></Fld>
            <Fld label="平台 *(可多选)" span={4}>
              <div className="flex flex-wrap gap-1">
                {INF_PLATFORMS.map(p=>{ const on=(f.platforms||[]).includes(p.v); return (
                  <button key={p.v} type="button" onClick={()=>togglePlat(p.v)} className={'px-2 py-0.5 text-xs rounded-md border '+(on?'bg-pink-100 border-pink-400 text-pink-800 font-semibold':'bg-white border-stone-300 text-stone-500')}>{on?'✓':''}{p.emoji} {p.label}</button>
                ); })}
              </div>
            </Fld>
          </div>
        ...
        <fieldset className="border border-stone-200 rounded-lg p-3 mb-3">
          <legend className="text-xs font-bold text-stone-700 px-1">📦 产品 + 合作</legend>
          <div className="grid grid-cols-4 gap-2">
            <Fld label="产品名" span={2}><input className={FIN} value={f.productName||''} onChange={e=>up('productName',e.target.value)} placeholder="Glimmering Wall Sconce" /></Fld>
            <Fld label="产品 SKU"><input className={FIN} value={f.productSku||''} onChange={e=>up('productSku',e.target.value)} placeholder="VKW7-25109-01" /></Fld>
            <Fld label="合作类型"><select className={FIN} value={f.collabType||'gifted'} onChange={e=>up('collabType',e.target.value)}>{INF_COLLAB.map(c=><option key={c.v} value={c.v}>{c.label}</option>)}</select></Fld>
            <Fld label="产品主图 URL" span={2}><input className={FIN} value={f.productImage||''} onChange={e=>up('productImage',e.target.value)} placeholder="https://cdn.shopify.com/..." /></Fld>
            <Fld label="付费金额 ¥"><input type="number" className={FIN} value={f.fee||''} onChange={e=>up('fee',e.target.value)} /></Fld>
            <Fld label="产品成本 ¥"><input type="number" className={FIN} value={f.sampleCost||''} onChange={e=>up('sampleCost',e.target.value)} /></Fld>
            <Fld label="运费 ¥"><input type="number" className={FIN} value={f.shippingCost||''} onChange={e=>up('shippingCost',e.target.value)} /></Fld>
            <Fld label="折扣码"><input className={FIN} value={f.discountCode||''} onChange={e=>up('discountCode',e.target.value)} placeholder="LANA15" /></Fld>
            <Fld label="物流单号"><input className={FIN} value={f.shippingTracking||''} onChange={e=>up('shippingTracking',e.target.value)} /></Fld>
            <Fld label="寄出日期"><input type="date" className={FIN} value={f.sentDate||''} onChange={e=>up('sentDate',e.target.value)} /></Fld>
            <Fld label="发布截止"><input type="date" className={FIN} value={f.deadline||''} onChange={e=>up('deadline',e.target.value)} /></Fld>
          </div>
        </fieldset>

        <fieldset className="border border-stone-200 rounded-lg p-3 mb-3">
          <legend className="text-xs font-bold text-stone-700 px-1">📝 Brief 内容</legend>
          <div className="grid grid-cols-4 gap-2">
            <Fld label="状态"><select className={FIN} value={f.status||'contacted'} onChange={e=>up('status',e.target.value)}>{INF_STATUSES.map(s=><option key={s.v} value={s.v}>{s.emoji} {s.label}</option>)}</select></Fld>
            <Fld label="内容评级"><select className={FIN} value={f.contentRating||''} onChange={e=>up('contentRating',e.target.value)}><option value="">—</option>{INF_RATINGS.map(r=><option key={r} value={r}>{r}</option>)}</select></Fld>
            <Fld label="Hashtags" span={2}><input className={FIN} value={f.hashtags||''} onChange={e=>up('hashtags',e.target.value)} placeholder="#vakkerlight #homedecor" /></Fld>
            <Fld label="发布要求" span={4}><input className={FIN} value={f.postingRequirements||''} onChange={e=>up('postingRequirements',e.target.value)} placeholder="Reel 30 秒以上" /></Fld>
            <Fld label="Brief 要点(每行一条)" span={2}><textarea className={FIN} rows={3} value={(f.briefPoints||[]).join('\n')} onChange={e=>up('briefPoints', e.target.value.split('\n').map(s=>s.trim()).filter(Boolean))} placeholder={'拍开灯效果\n近景拍材质纹理'} /></Fld>
            <Fld label="Do's & Don'ts" span={2}><textarea className={FIN} rows={3} value={f.dosDonts||''} onChange={e=>up('dosDonts',e.target.value)} placeholder="✓ 开灯拍  ✗ 不要加第三方滤镜" /></Fld>
          </div>
        </fieldset>

        <fieldset className="border border-stone-200 rounded-lg p-3 mb-1">
          <legend className="text-xs font-bold text-stone-700 px-1">📊 发布后数据(完成时填)</legend>
          <div className="grid grid-cols-4 gap-2">
            <Fld label="已发布内容链接(每行一条)" span={2}><textarea className={FIN} rows={2} value={(f.contentUrls||[]).join('\n')} onChange={e=>up('contentUrls', e.target.value.split('\n').map(s=>s.trim()).filter(Boolean))} placeholder="https://instagram.com/p/..." /></Fld>
            <Fld label="备注" span={2}><textarea className={FIN} rows={2} value={f.notes||''} onChange={e=>up('notes',e.target.value)} /></Fld>
            <Fld label="👁 浏览"><input type="number" className={FIN} value={m.views||''} onChange={e=>upMetric('views',e.target.value)} /></Fld>
            <Fld label="❤️ 点赞"><input type="number" className={FIN} value={m.likes||''} onChange={e=>upMetric('likes',e.target.value)} /></Fld>
            <Fld label="🔗 点击"><input type="number" className={FIN} value={m.clicks||''} onChange={e=>upMetric('clicks',e.target.value)} /></Fld>
            <Fld label="💰 转化(单)"><input type="number" className={FIN} value={m.sales||''} onChange={e=>upMetric('sales',e.target.value)} /></Fld>
          </div>
        </fieldset>

        <div className="flex items-center justify-between mt-3 pt-3 border-t border-stone-200 sticky bottom-0 bg-white">
          <button onClick={briefSheet} className="text-sm text-pink-600 hover:text-pink-700 font-medium">📋 Brief Sheet 预览/复制</button>
          <div className="flex gap-2">
            <button onClick={onCancel} className="px-4 py-2 rounded-lg border border-stone-300 text-stone-600">取消</button>
            <button onClick={()=>onSave(f)} disabled={saving} className="px-4 py-2 rounded-lg bg-pink-600 text-white disabled:opacity-50">{saving?'保存中…':'保存'}</button>
          </div>
        </div>
      </div>
    </div>
...
const InfluencerModule = ({ user, employees = [], toast }) => {
  ...
  const blankForm = () => ({ platforms:[], collabType:'gifted', status:'contacted', followerCount:0, sampleCost:0, shippingCost:0, fee:0, briefPoints:[], contentUrls:[], metrics:{} });

  const load = async () => {
    const c = getCdmClient(); if(!c){ setLoading(false); toast && toast('⚠️ 消息总线未连接'); return; }
    setLoading(true);
    try {
      const { data, error } = await c.from('influencer_briefs').select('*').order('created_at_ms',{ascending:false}).limit(1000);
      if(error) throw error;
      const mapped = (data||[]).map(r => ({
        id:r.id, influencerName:r.influencer_name,
        platforms: infSafeArr(r.platforms).length ? infSafeArr(r.platforms) : (r.platform ? [r.platform] : []),
        handle:r.handle, profileUrl:r.profile_url, followerCount:r.follower_count||0,
        region:r.region, niche:r.niche, contact:r.contact,
        productSku:r.product_sku, productName:r.product_name, productImage:r.product_image,
        sampleCost:Number(r.sample_cost)||0, shippingCost:Number(r.shipping_cost)||0,
        shippingTracking:r.shipping_tracking, sentDate:r.sent_date,
        collabType:r.collab_type||'gifted', fee:Number(r.fee)||0, discountCode:r.discount_code,
        briefPoints:infSafeArr(r.brief_points), hashtags:r.hashtags,
        postingRequirements:r.posting_requirements, deadline:r.deadline, dosDonts:r.dos_donts,
        status:r.status||'contacted', contentUrls:infSafeArr(r.content_urls), metrics:infSafeObj(r.metrics),
        contentRating:r.content_rating, notes:r.notes, attachments:infSafeArr(r.attachments),
        createdById:r.created_by_id, createdByName:r.created_by_name, createdAtMs:r.created_at_ms
      }));
      setBriefs(mapped);
    } catch(e){ console.error('[influencer] load', e); toast && toast('⚠️ 加载影响者数据失败'); }
    setLoading(false);
  };

  useEffect(() => { load(); }, []);
  useEffect(() => {
    const c = getCdmClient(); if(!c) return;
    const ch = c.channel('influencer-realtime')
      .on('postgres_changes', { event:'*', schema:'public', table:'influencer_briefs' }, () => load())
      .subscribe();
    return () => { try { c.removeChannel(ch); } catch(e){} };
  }, []);

  const save = async (b) => {
    if(!b.influencerName || !b.influencerName.trim()){ toast && toast('⚠️ 请填博主名'); return; }
    setSaving(true);
    const c = getCdmClient(); if(!c){ setSaving(false); toast && toast('⚠️ 未连接'); return; }
    const platformsArr = Array.isArray(b.platforms)&&b.platforms.length ? b.platforms : (b.platform?[b.platform]:[]);
    const row = {
      id: b.id || crypto.randomUUID(),
      influencer_name: b.influencerName.trim(),
      platforms: platformsArr, platform: platformsArr[0] || null,
      handle:b.handle||null, profile_url:b.profileUrl||null, follower_count:Number(b.followerCount)||0,
      region:b.region||null, niche:b.niche||null, contact:b.contact||null,
      product_sku:b.productSku||null, product_name:b.productName||null, product_image:b.productImage||null,
      sample_cost:Number(b.sampleCost)||0, shipping_cost:Number(b.shippingCost)||0,
      shipping_tracking:b.shippingTracking||null, sent_date:b.sentDate||null,
      collab_type:b.collabType||'gifted', fee:Number(b.fee)||0, discount_code:b.discountCode||null,
      brief_points:Array.isArray(b.briefPoints)?b.briefPoints:[], hashtags:b.hashtags||null,
      posting_requirements:b.postingRequirements||null, deadline:b.deadline||null, dos_donts:b.dosDonts||null,
      status:b.status||'contacted', content_urls:Array.isArray(b.contentUrls)?b.contentUrls:[],
      metrics:b.metrics||{}, content_rating:b.contentRating||null, notes:b.notes||null,
      attachments:Array.isArray(b.attachments)?b.attachments:[],
      created_by_id:b.createdById||user.id, created_by_name:b.createdByName||user.name||null,
      created_at_ms:b.createdAtMs||Date.now(), updated_at:new Date().toISOString()
    };
    try { const { error } = await c.from('influencer_briefs').upsert(row); if(error) throw error;
      toast && toast('✓ 已保存'); setEditing(null); load();
    } catch(e){ console.error('[influencer] save', e); toast && toast('⚠️ 保存失败:'+(e.message||'')); }
    setSaving(false);
  };

  const changeStatus = async (b, st) => {
    const c = getCdmClient(); if(!c) return;
    setBriefs(prev => prev.map(x => x.id===b.id ? {...x, status:st} : x));
    try { const { error } = await c.from('influencer_briefs').update({ status:st, updated_at:new Date().toISOString() }).eq('id', b.id); if(error) throw error;
      toast && toast('✓ 状态 → '+infStat(st).emoji+' '+infStat(st).label);
    } catch(e){ toast && toast('⚠️ 改状态失败'); load(); }
  };

  const del = async (b) => {
    if(!await wsConfirm('确定删除「'+b.influencerName+'」这条合作?不可恢复')) return;
    const c = getCdmClient(); if(!c) return;
    try { const { error } = await c.from('influencer_briefs').delete().eq('id', b.id); if(error) throw error;
      setBriefs(prev=>prev.filter(x=>x.id!==b.id)); toast && toast('✓ 已删除');
    } catch(e){ toast && toast('⚠️ 删除失败'); }
  };

  const filtered = briefs.filter(b => {
    if(fStatus!=='all' && b.status!==fStatus) return false;
    if(fPlatform!=='all' && !(b.platforms||[]).includes(fPlatform)) return false;
    if(search.trim()){ const q=search.toLowerCase(); const hay=((b.influencerName||'')+' '+(b.handle||'')+' '+(b.productName||'')+' '+(b.discountCode||'')+' '+(b.niche||'')).toLowerCase(); if(!hay.includes(q)) return false; }
    return true;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <h2 className="text-xl font-bold text-stone-800">✨ 影响者合作</h2>
        <span className="text-xs px-2 py-0.5 rounded-full bg-pink-100 text-pink-700">{briefs.length} 条</span>
        <span className="text-xs text-stone-400">与美工 WorkTrack 共享 · 实时同步</span>
        <span className="flex-1"></span>
        <div className="flex rounded-lg overflow-hidden border border-stone-300">
          <button onClick={()=>setView('pipeline')} className={'px-3 py-1.5 text-sm '+(view==='pipeline'?'bg-pink-600 text-white':'bg-white text-stone-600')}>🎬 流水线</button>
          <button onClick={()=>setView('list')} className={'px-3 py-1.5 text-sm '+(view==='list'?'bg-pink-600 text-white':'bg-white text-stone-600')}>📋 列表</button>
        </div>
        <button onClick={()=>setEditing(blankForm())} className="px-3 py-1.5 text-sm rounded-lg bg-stone-800 text-white">+ 新建合作</button>
      </div>
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 博主 / 产品 / 折扣码 / niche" className="px-3 py-1.5 text-sm border border-stone-300 rounded-lg" style={{minWidth:200}} />
        <select value={fStatus} onChange={e=>setFStatus(e.target.value)} className="px-2 py-1.5 text-sm border border-stone-300 rounded-lg">
          <option value="all">全部状态</option>{INF_STATUSES.map(s=><option key={s.v} value={s.v}>{s.emoji} {s.label}</option>)}
        </select>
        <select value={fPlatform} onChange={e=>setFPlatform(e.target.value)} className="px-2 py-1.5 text-sm border border-stone-300 rounded-lg">
          <option value="all">全部平台</option>{INF_PLATFORMS.map(p=><option key={p.v} value={p.v}>{p.emoji} {p.label}</option>)}
        </select>
        <span className="text-xs text-stone-400">显示 {filtered.length} / {briefs.length}</span>
      </div>

      {loading ? <div className="text-center text-stone-400 py-12">加载中…</div> : (
        view==='pipeline' ? (
          <div className="grid gap-1.5" style={{gridTemplateColumns:'repeat(7,minmax(0,1fr))'}}>
            {INF_STATUSES.map(col => {
              const items = filtered.filter(b=>b.status===col.v);
              return (
                <div key={col.v} className="bg-stone-50 rounded-lg p-1.5" style={{minHeight:380}}>
                  <div className="text-xs font-bold mb-2 px-1.5 py-1 rounded flex items-center justify-between" style={{background:col.bg, color:col.color}}><span className="truncate">{col.emoji} {col.label}</span><span className="opacity-60 ml-1">{items.length}</span></div>
                  <div className="space-y-1.5">
                    {items.map(b => { const emojis=(b.platforms||[]).map(p=>infPlat(p).emoji).join(''); return (
                      <button key={b.id} onClick={()=>setEditing(b)} className="w-full text-left bg-white rounded-md p-1.5 border border-stone-200 hover:border-pink-300">
                        <div className="text-[11px] font-bold truncate text-stone-800">{emojis} {b.influencerName}</div>
                        <div className="text-[10px] text-stone-500 truncate">{b.productName||'—'}</div>
                        {(b.followerCount>0||(b.sampleCost+b.shippingCost+b.fee)>0)&&<div className="text-[9px] text-stone-400 truncate">{b.followerCount>0?(b.followerCount>=1000?(Math.round(b.followerCount/100)/10)+'k 粉':b.followerCount+' 粉'):''}{(b.sampleCost+b.shippingCost+b.fee)>0?` · ¥${b.sampleCost+b.shippingCost+b.fee}`:''}</div>}
                      </button>
                    ); })}
                    {items.length===0 && <div className="text-[10px] text-stone-300 text-center py-3 border border-dashed border-stone-200 rounded">—</div>}
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid gap-3" style={{gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))'}}>
            {filtered.map(b => <InfCard key={b.id} b={b} onOpen={()=>setEditing(b)} onStatus={st=>changeStatus(b,st)} onDel={()=>del(b)} />)}
            {filtered.length===0 && <div className="text-stone-400 py-8 col-span-full text-center">没有符合条件的合作 · 点「+ 新建合作」开始</div>}
          </div>
        )
      )}

      {editing && <InfForm initial={editing} onCancel={()=>setEditing(null)} onSave={save} saving={saving} toast={toast} />}
    </div>
  );
};
...
function cdmCompressImg(dataUrl, max = 1280, quality = 0.8) {
...
const CrossDeptModule = ({ user, employees, messages, loading, onReload, toast, shopOwners = [], cdmTimeoutConfig = {} }) => {
  ...
  const openMsgBase = useMemo(() => (openMsgId ? messages.find(m => m.id === openMsgId) : null), [openMsgId, messages]);
  // 🆕 fix136: 详情按需补取 attachments/thread(列表已不带,避免巨型 base64 拖慢)
  const [openMsgExtra, setOpenMsgExtra] = useState(null);
  useEffect(() => {
    ...
    (async () => {
      try {
        const c = getCdmClient(); if (!c) return;
        const { data } = await c.from('cross_dept_messages').select('attachments,thread').eq('id', openMsgId).maybeSingle();
  ...
  const openMsg = useMemo(() => {
    ...
    return { ...openMsgBase, attachments: (openMsgExtra && openMsgExtra.attachments) || openMsgBase.attachments || [], thread: (openMsgExtra && openMsgExtra.thread) || openMsgBase.thread || [] };
...
const CdmNewMessageModal = ({ user, employees, shopOwners = [], onClose, onSent, toast }) => {
  ...
  const recipientOptions = useMemo(() => {
    const fromDir = orgDir.filter(p => p.system === toSystem && p.active).sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
    if (fromDir.length) return fromDir;
    const seen = new Set(); const out = [];
    (shopOwners || []).filter(s => s.system === toSystem && s.userId && s.userName).forEach(s => {
      if (!seen.has(s.userId)) { seen.add(s.userId); out.push({ id: s.userId, staffId: s.userId, name: s.userName, role: '', system: toSystem, active: true }); }
    });
  ...
  const readFiles = async (files) => {
    ...
    for (const f of files) {
      ...
      let du = dataUrl;
      if ((f.type || '').startsWith('image/') || /^data:image\//i.test(dataUrl)) { du = await cdmCompressImg(dataUrl); }
      arr.push({ name: f.name, mime: f.type || 'image/jpeg', dataUrl: du, size: (du && du.length) || f.size });
    }
...
const CdmDetailModal = ({ msg, user, employees = [], shopOwners = [], cdmTimeoutConfig = {}, onClose, onChanged, toast }) => {
  ...
  const setStatus = async (newStatus) => {
    ...
    if (newStatus === 'done') {
      ...
      try {
        const { data: latest } = await client.from('cross_dept_messages').select('thread, attachments').eq('id', msg.id).maybeSingle();
        const curThread = (latest && Array.isArray(latest.thread)) ? latest.thread : (Array.isArray(msg.thread) ? msg.thread : []);
        const topAtts = (latest && Array.isArray(latest.attachments)) ? latest.attachments : (Array.isArray(msg.attachments) ? msg.attachments : []);
        const threadAtts = [];
        curThread.forEach(t => { if (t && t.system === MY_SYSTEM && Array.isArray(t.attachments)) threadAtts.push(...t.attachments); });
        const seen = new Set(); const merged = [];
        [...topAtts, ...threadAtts].forEach(a => { const u = a && (a.url || a.dataUrl); if (u && !seen.has(u)) { seen.add(u); merged.push(a); } });
        updates.attachments = merged;
        const attNames = merged.map(a => a && (a.name || '附件')).filter(Boolean);
        updates.thread = [...curThread, {
          user_id: user.id, user_name: userName, system: MY_SYSTEM, ts: Date.now(),
          content: '✅ 已完成' + (attNames.length ? ' · 详见附件:' + attNames.join('、') : ''),
        }];
      } catch (aggErr) { console.warn('[CDM] 完成聚合失败', aggErr); }
    } else {
      // 撤销完成 — 清空完成字段
  ...
  return ReactDOM.createPortal(
    <div onClick={onClose} style={{position:'fixed', inset:0, background:'rgba(0,0,0,.5)', zIndex:100000, display:'flex', alignItems:'flex-start', justifyContent:'center', overflow:'auto', padding:'30px 20px'}}>
      ...
      {/* 图片预览 */}
      {imgPreview && (
        <div onClick={() => setImgPreview(null)} style={{position:'fixed', inset:0, background:'rgba(0,0,0,.85)', zIndex:100020, display:'flex', alignItems:'center', justifyContent:'center', padding:30, cursor:'zoom-out'}}>
...
const ShopOwnersManager = ({ user, employees, shopOwners = [], onClose, toast }) => {
  ...
  const deleteOwner = async (owner) => {
    ...
    if (!await wsConfirm(`删除「${owner.shopName} - ${owner.userName}」?`)) return;
    const client = getCdmClient();
    if (!client) { alert('消息总线未连接'); return; }
    try {
...
const TimeoutSettingsModal = ({ user, cdmTimeoutConfig = {}, onClose, toast }) => {
  ...
  const resetAll = async () => {
    if (!await wsConfirm('确定要把客服部所有分类的超时阈值都恢复默认?')) return;
    setConfig({});
    toast('✓ 全部已恢复默认');
  };
  const save = async () => {
...
const PhotoRequestsModule = ({ user, toast }) => {
  ...
  const [loadedCount, setLoadedCount] = useState(PHOTO_PAGE);
  ...
  const refresh = async (count) => {
    ...
    setLoading(true);
    try {
      const want = count || loadedCount || PHOTO_PAGE;
      // 重拉当前已加载量(分页 range),不再一次性全拉
      const data = await window.listPhotoRequests({ limit: want });
      setList(data || []);
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const next = loadedCount + PHOTO_PAGE;
      const data = await window.listPhotoRequests({ limit: next });
      setList(data || []);
      setHasMore((data || []).length >= next);
      setLoadedCount(next);
    } catch (e) { console.error('[PhotoReq] 加载更多失败', e); }
    setLoadingMore(false);
  };

  useEffect(() => { refresh(PHOTO_PAGE); }, []);
        ...
        {visible.length === 0 ? (
          ...
          <>
            ...
            <div style={{
              ...
              <span>📊 当前显示 <strong style={{color:'var(--ink-2)'}}>{visible.length}</strong> / 共 {list.length} 条{hasMore ? '+' : ''}</span>
              ...
              <div style={{flex:1}}/>
              {hasMore ? (
                <button onClick={loadMore} disabled={loadingMore} className="btn-sec" style={{fontSize:12, padding:'5px 14px'}}>
                  {loadingMore ? '加载中…' : `加载更多 (+${PHOTO_PAGE})`}
                </button>
              ) : (
...
const PhotoRequestNewModal = ({ user, toast, onClose, onSuccess, prefill = {} }) => {
  ...
  const submit = async () => {
    ...
    if (shops.length === 0) {
      if (!await wsConfirm('没选店铺,拍摄部可能不知道用在哪。继续提交?')) return;
    }
    setSubmitting(true);
    try {
