  ...
  return (
    <div onClick={onClickCard} title="点击切回列表视图编辑这条"
      ...
      <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', fontSize:12}}>
        {orderRef && (() => {
          const _u = wsOrderAdminUrl(orderRef);
          const _txt = '🧾 ' + (orderRef.length > 28 ? orderRef.slice(0, 28) + '…' : orderRef);
          return _u
            ? <a href={_u} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} title="打开订单后台页"
                style={{padding:'2px 8px', background:'var(--bg-elevated)', borderRadius:6, fontFamily:'SF Mono,monospace', color:'var(--accent)', textDecoration:'none'}}>{_txt} ↗</a>
            : <span style={{padding:'2px 8px', background:'var(--bg-elevated)', borderRadius:6, fontFamily:'SF Mono,monospace', color:'var(--ink-2)'}}>{_txt}</span>;
        })()}
        {shopName && (
      ...
      {shots.length > 0 && (
        <div style={{display:'flex', gap:6, flexWrap:'wrap'}}>
          {shots.slice(0, 4).map((s, i) => (
            <img key={s.id || i} src={imgDisplaySrc(s)} alt={s.name || ''} className="img-thumb" loading="lazy"
              onClick={e => { e.stopPropagation(); (window.__setPreviewImg || onViewImg || (()=>{}))(s); }}
              onError={e => { e.currentTarget.style.display = 'none'; }}
              style={{width:56, height:56, objectFit:'contain', borderRadius:8, border:'1px solid var(--line)', background:'var(--bg-elevated)', cursor:'zoom-in'}}/>
          ))}
          {shots.length > 4 && (
      ...
      {/* 🆕 fix141: 反馈截图 feedbackShots 也铺缩略图 */}
      {Array.isArray(r.feedbackShots) && r.feedbackShots.length > 0 && (
        <AttachThumbs files={r.feedbackShots} size={48} max={6} onPreview={window.__setPreviewImg} />
      )}
...
const CSModule = ({ user, employees, records, setRecords, onTrash, toast, cloudOn, focusId, onOpenQuoteTab }) => {
  ...
  const csFocusRef = useRef(null);
  const [flashId, setFlashId] = useState(null);
  useEffect(() => {
    if (!focusId || focusId === csFocusRef.current) return;
    const rec = (records || []).find(x => x.id === focusId);
    if (!rec) return;  // 数据还没到/找不到 → 不动,等加载好再触发
    csFocusRef.current = focusId;
    setCardViewPersist('list');
    setViewMode('all'); setDateRange('all'); setFilterStatus('all'); setFilterSite('all'); setFilterCategory('all');
  ...
  useEffect(() => {
    if (!flashId) return;
    const tries = [120, 400, 900];
    const timers = tries.map(ms => setTimeout(() => {
      const el = document.getElementById('cs-row-' + flashId);
      if (el) {
  ...
  const [quoteReminders, setQuoteReminders] = useState([]);
  useEffect(() => {
    if (!cloudOn) return;
    let cancelled = false;
    (async () => {
      // 只取轻量列 + 用 JSON 子字段取跟进标记/日期(不拉 data 大图);若子字段查询不支持会回退 * 再兜底从 data 读
      const rows = await CLOUD.list('quotations', {
      ...
      const mapped = rows.map(q => {
        const nf = q.need_followup !== undefined ? q.need_followup : (q.data && q.data.need_followup);
        const fd = q.followup_date !== undefined ? q.followup_date : (q.data && q.data.followup_date);
        return { q, nf: (nf === true || nf === 'true'), fd: (fd || '').trim() };
      }).filter(x => x.nf && x.fd && !x.q.trashed && !['won', 'lost', 'archived'].includes(x.q.status))
        .map(({ q, fd }) => {
          const owner = employees.find(e => [e.alias, e.name, ((e.name || '') + ' ' + (e.alias || '')).trim()].includes(q.created_by_name));
          const amt = q.amount != null && q.amount !== '' ? (q.currency || 'USD') + ' ' + Number(q.amount).toLocaleString() : '';
          return {
            id: 'quote_' + q.id, isQuote: true, quoteId: q.id,
            nextFollowUp: fd, status: 'following',
            ownerId: owner ? owner.id : null, ownerName: q.created_by_name || '',
            customer: q.buyer_name || '', orderRef: q.pi_no || '',
            note: '报价单' + (amt ? ' · ' + amt : ''), site: '报价',
          };
        });
  ...
  const reminders = useMemo(() => {
    ...
    const base = isAdmin ? allLive : allLive.filter(r => r.ownerId === user.id);
    const q = isAdmin ? quoteReminders : quoteReminders.filter(r => r.ownerId === user.id);
    return [...base, ...q];
  ...
  const deleteRow = async (id) => {
    if (!await wsConfirm('确认放入回收站？主管可恢复。')) return;
  ...
  const [reminderViewer, setReminderViewer] = useState(isAdmin ? 'all' : 'self');
  const [starredStaff, setStarredStaff] = useState(() => {
    try { return JSON.parse(localStorage.getItem('cs_starred_staff') || '[]'); } catch (e) { return []; }
  });
  const toggleStar = (id) => setStarredStaff(prev => {
    const next = prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id];
    try { localStorage.setItem('cs_starred_staff', JSON.stringify(next)); } catch (e) {}
    return next;
  });
  // 转交 modal
  ...
  const sortedReminders = useMemo(() => {
    let list = reminders;
    if (reminderViewer === 'self') list = reminders.filter(r => r.ownerId === user.id);
    else if (reminderViewer !== 'all') list = reminders.filter(r => r.ownerId === reminderViewer); // 单独看某人
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isAdmin && (
              <span className="flex items-center gap-1.5 flex-wrap" onClick={e => e.stopPropagation()}>
                <select value={reminderViewer} onChange={e => setReminderViewer(e.target.value)}
                  style={{fontSize:11, padding:'2px 8px', borderRadius:6, border:'1px solid var(--gold)', color:'var(--gold)', background:'rgba(202,138,4,.08)', fontWeight:600, cursor:'pointer'}}>
                  <option value="all">👑 全部员工</option>
                  <option value="self">🙋 仅本人</option>
                  {[...employees].filter(e => !e.hideFromList && e.role !== 'super_admin')
                    .sort((a,b) => (starredStaff.includes(b.id)?1:0) - (starredStaff.includes(a.id)?1:0))
                    .map(e => <option key={e.id} value={e.id}>{starredStaff.includes(e.id) ? '⭐ ' : ''}{e.name}{e.alias ? ' ' + e.alias : ''}</option>)}
                </select>
                {reminderViewer !== 'all' && reminderViewer !== 'self' && (
                  <button className="btn-ghost" style={{padding:'2px 8px', fontSize:11}}
                    onClick={() => toggleStar(reminderViewer)} title="标星此人,排到列表前 / 快速查看">
                    {starredStaff.includes(reminderViewer) ? '⭐ 已星标' : '☆ 标星'}
                  </button>
                )}
                ...
                <div key={r.id} className="row-hover" style={{display:'flex', alignItems:'center', gap:12, padding:'10px 16px', borderBottom:'1px solid var(--line)', fontSize:13}}>
                  ...
                  <div style={{flex:1, minWidth:0}}>
                    <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
                      ...
                      {r.site && <span className="tag" style={{fontSize:10, padding:'1px 6px', ...(r.isQuote ? {background:'#ede9fe', color:'#6d28d9', fontWeight:600} : {})}}>{r.isQuote ? '📄 报价' : r.site}</span>}
                      <span style={{color:'var(--ink-3)', fontSize:11}}>👤 {owner ? (owner.name + (owner.alias ? ' ' + owner.alias : '')) : (r.ownerName || '?')}</span>
                      {lastTransfer && (
                  ...
                  <div style={{display:'flex', gap:6, flexShrink:0}}>
                    {r.isQuote ? (
                      <button className="btn-sec" style={{padding:'4px 10px', fontSize:11}} onClick={() => onOpenQuoteTab && onOpenQuoteTab()} title="打开报价单模块">
                        📄 查看报价单
                      </button>
                    ) : (
                      <>
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isAdmin && isPastDate(viewDate) && (
              <button className="btn-ghost text-[11px]" onClick={async () => {
                if (!adminUnlock) {
                  if (await wsConfirm('解锁历史日期编辑？\n\n注意：仅主管可执行此操作。修改历史记录将影响绩效统计与审计追溯，请谨慎操作。')) {
                    setAdminUnlock(true);
      ...
      {followUpModal && (() => {
        ...
        return (
          <FollowUpModal
            ...
            onDeleteEvent={async (kind, evt) => {
              const table = kind === 'aftersale' ? 'aftersales' : kind === 'refill' ? 'refills' : 'refunds';
              const label = kind === 'aftersale' ? '🔧 售后' : kind === 'refill' ? '📦 补件' : '💰 退款';
              await requestDelete({
                user, tableName: table, moduleLabel: label, record: evt,
                recordSummary: `${evt.order_ref || r.orderRef || '?'} · ${evt.product_name || evt.customer || ''}`,
                toast,
                onSuccess: () => {
                  if (kind === 'aftersale') setAftersales(prev => prev.filter(a => a.id !== evt.id));
                  else if (kind === 'refill') setRefills(prev => prev.filter(x => x.id !== evt.id));
                  else setRefunds(prev => prev.filter(x => x.id !== evt.id));
                },
              });
            }}
          />
...
const getFileTypeInfo = (fileObj) => {
  ...
  const mime = fileObj.type || fileObj.mime || '';
  const urlStr = (fileObj.url || fileObj.dataUrl || '').toLowerCase();
  const ext = name.split('.').pop()?.toLowerCase() || '';
  const isImgUrl = /^data:image\//i.test(fileObj.url || fileObj.dataUrl || '') || /\.(jpg|jpeg|png|gif|webp|svg|bmp)(\?|$)/i.test(urlStr);
  const isVidUrl = /\.(mp4|webm|mov|avi|m4v)(\?|$)/i.test(urlStr);
...
const FilePreviewModal = ({ file, onClose }) => {
  ...
  return (
    <div onClick={onClose}
      style={{position:'fixed', inset:0, background:'rgba(0,0,0,.85)', zIndex:2000000, display:'flex', flexDirection:'column', padding:20, gap:10}}>
...
const MultiFileUploader = ({ files, setFiles, bucket = 'briefing-files', maxSize = 50, accept = '*' }) => {
  ...
  const removeFile = async (file) => {
    if (!await wsConfirm(`删除 "${file.name}"？`)) return;
    ...
    <div>
      ...
      {files && files.length > 0 && (
        <div style={{marginTop:10}}>
          <div style={{fontSize:11, color:'#15803d', fontWeight:600, marginBottom:6}}>✅ 已添加 {files.length} 个文件 · 保存表单时自动一并存入 · 点缩略图原地展开看大图</div>
          <div style={{display:'flex', flexDirection:'column', gap:6}}>
          {files.map(f => {
            const info = getFileTypeInfo(f);
            const isImg = info.kind === 'image';
            const isOpen = expandedId === f.id;
            const toggle = () => { if (isImg) setExpandedId(isOpen ? null : f.id); else window.open(f.url, '_blank'); };
            return (
              <div key={f.id} style={{background:'white', border:'1px solid var(--line)', borderRadius:8, overflow:'hidden'}}>
                <div style={{display:'flex', alignItems:'center', gap:10, padding:8}}>
                  {isImg ? (
                    <img src={f.url} style={{width:40, height:40, objectFit:'cover', borderRadius:5, flexShrink:0, cursor:'pointer'}} onClick={toggle} />
                  ) : (
                    <div style={{width:40, height:40, background: info.color + '15', borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, flexShrink:0, cursor:'pointer'}} onClick={toggle}>
                      {info.icon}
                    </div>
                  )}
                  ...
                  {isImg ? (
                    ...
                    <a href={f.url} target="_blank" rel="noreferrer"
                      style={{padding:'4px 10px', background:'#e0f2fe', color:'#0369a1', border:'1px solid #7dd3fc', borderRadius:5, fontSize:11, fontWeight:600, textDecoration:'none', flexShrink:0}}>↗ 打开</a>
                  )}
                ...
                {isOpen && isImg && (
                  <div style={{padding:10, borderTop:'1px solid var(--line)', background:'#111', textAlign:'center'}}>
                    <img src={f.url} alt={f.name} style={{maxWidth:'100%', maxHeight:380, borderRadius:6, objectFit:'contain'}} />
                    <div style={{marginTop:6}}><button type="button" onClick={() => wsOpenImg(f.url)} style={{background:'none', border:'none', fontSize:11, color:'#7dd3fc', cursor:'pointer', padding:0}}>↗ 在新标签打开原图</button></div>
                  </div>
                )}
              </div>
            );
...
const MultiImageUploader = ({ attachments, setAttachments, defaultLabel = 'product', maxSize = 10, acceptVideo = false, videoMaxSize = 100 }) => {
  ...
  const uploadFile = async (file, label = defaultLabel) => {
    ...
    try {
      ...
      if (!res) {
        const msg = (CLOUD._lastError && CLOUD._lastError.message) || '';
        const sizeHint = (isVid || /size|large|exceed|payload|413|maximum/i.test(msg))
  ...
  const removeAttach = async (att) => {
    if (!await wsConfirm(isVideo(att) ? '删除这个视频?' : '删除这张图?')) return;
  ...
  return (
    <div>
      ...
      {/* 🆕 fix126: 图片放大 — 行内展开(无浮层,永不被表单压背面)*/}
      {previewIdx !== null && attachments[previewIdx] && (
        <div style={{marginTop:10, border:'1px solid var(--line)', borderRadius:10, overflow:'hidden', background:'#111'}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 10px', background:'#1c1c1e'}}>
            <span style={{color:'#d1d5db', fontSize:11}}>{previewIdx + 1} / {attachments.length} · {attachments[previewIdx].name || ''}</span>
            <div style={{display:'flex', gap:8, alignItems:'center'}}>
              {attachments.length > 1 && (
                <>
                  <button type="button" onClick={() => setPreviewIdx((previewIdx - 1 + attachments.length) % attachments.length)} style={{padding:'2px 8px', background:'rgba(255,255,255,.15)', color:'white', border:'none', borderRadius:5, cursor:'pointer', fontSize:11}}>←</button>
                  <button type="button" onClick={() => setPreviewIdx((previewIdx + 1) % attachments.length)} style={{padding:'2px 8px', background:'rgba(255,255,255,.15)', color:'white', border:'none', borderRadius:5, cursor:'pointer', fontSize:11}}>→</button>
                </>
              )}
              <button type="button" onClick={() => wsOpenImg(attachments[previewIdx].url)} style={{background:'none', border:'none', fontSize:11, color:'#7dd3fc', cursor:'pointer', padding:0}}>↗ 原图</button>
              <button type="button" onClick={() => setPreviewIdx(null)} style={{padding:'2px 9px', background:'rgba(255,255,255,.15)', color:'white', border:'none', borderRadius:5, cursor:'pointer', fontSize:11}}>收起 ▲</button>
            </div>
          </div>
          <div style={{textAlign:'center', padding:10}}>
            {isVideo(attachments[previewIdx])
              ? <video src={attachments[previewIdx].url} controls autoPlay style={{maxWidth:'100%', maxHeight:380, background:'black', borderRadius:6}} />
              : <img src={attachments[previewIdx].url} alt="" style={{maxWidth:'100%', maxHeight:380, objectFit:'contain', borderRadius:6}} />}
          </div>
        </div>
      )}
    </div>
...
const EventEditorModal = ({ kind, record, suppliers, user, onClose, onSaved, existingEvent = null }) => {
  ...
  const [improvementSuggestion, setImprovementSuggestion] = useState(existingEvent?.improvement_suggestion || '');
  const [improvementStatus, setImprovementStatus] = useState(existingEvent?.improvement_status || '');
  const [commImages, setCommImages] = useState(existingEvent?.communication_images || []);
  ...
  const [orderPull, setOrderPull] = useState({ loading: false, products: null, meta: null });
  const doPullOrder = async () => {
    const on = (orderRef || '').trim();
    if (!on) { alert('请先填订单号'); return; }
    setOrderPull({ loading: true, products: null, meta: null });
    try {
      const v = await wsFetchOrderProducts(on);
      setOrderPull({ loading: false, products: (v && v.products) || [], meta: v || null });
    ...
    } catch (e) { setOrderPull({ loading: false, products: [], meta: null }); }
  };
  const isProdSaved = (p) => (attachments || []).some(a => a.url === p.image_url);
  const pickPulledProduct = (p) => {
    if (p.title) setProductName(p.title);
    if (p.image_url) {
      if (isProdSaved(p)) {
        setAttachments((attachments || []).filter(a => a.url !== p.image_url));  // 再点取消保存
      } else {
        setAttachments([...(attachments || []), {
          id: 'a_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7),
          url: p.image_url, label: '产品图', kind: 'image', mime: 'image/jpeg',
          name: (p.title || 'product') + '.jpg', from_order: true, uploaded_at: new Date().toISOString(),
        }]);
      }
    }
        ...
        <div style={{padding:'18px 22px', overflowY:'auto', overflowX:'hidden', flex:1}}>
            ...
            <div>
              ...
              <div style={{display:'flex', gap:6}}>
                <input value={orderRef} onChange={e => setOrderRef(e.target.value)} placeholder="V99299"
                  style={{flex:1, minWidth:0, padding:'6px 10px', border:'1px solid var(--line)', borderRadius:6, fontSize:13}} />
                <button type="button" onClick={doPullOrder} disabled={orderPull.loading} title="从跟单库/Shopify 拉取该订单的产品,选一个直接填产品名+产品图(免手动上传)"
                  style={{whiteSpace:'nowrap', padding:'6px 10px', border:'none', borderRadius:6, background:'#0071e3', color:'white', fontSize:12, fontWeight:600, cursor:'pointer', flexShrink:0}}>
                  {orderPull.loading ? '拉取中…' : '🔄 拉取订单'}
                </button>
              </div>
              {orderPull.products && (
                <div style={{marginTop:8, padding:8, background:'#f8fafc', border:'1px solid var(--line)', borderRadius:8}}>
                  {orderPull.products.length === 0 ? (
                    <div style={{fontSize:11, color:'var(--ink-4)', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                      <span>没拉到该订单的产品(检查订单号/网站前缀,或仍可手动上传图片)</span>
                      <button type="button" onClick={() => setOrderPull({loading:false, products:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11}}>收起</button>
                    </div>
                  ) : (
                    <>
                      <div style={{fontSize:11, color:'var(--ink-3)', marginBottom:6, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <span>拉到 {orderPull.products.length} 个产品 · 点选 = 填产品名并存入附件(再点取消) · 已存 {(attachments||[]).filter(a=>a.from_order).length} 张</span>
                        <button type="button" onClick={() => setOrderPull({loading:false, products:null, meta:null})} style={{border:'none', background:'none', color:'var(--accent)', cursor:'pointer', fontSize:11, fontWeight:600}}>完成 ✓</button>
                      </div>
                      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(96px,1fr))', gap:8}}>
                        {orderPull.products.map((p, i) => {
                          const saved = isProdSaved(p);
                          return (
                          <div key={i} onClick={() => pickPulledProduct(p)} title={p.title || ''}
                            style={{position:'relative', border: saved ? '2px solid #16a34a' : '1px solid var(--line)', borderRadius:8, padding:5, cursor:'pointer', textAlign:'center', background: saved ? '#f0fdf4' : 'white'}}>
                            {saved && <div style={{position:'absolute', top:-7, right:-7, width:20, height:20, borderRadius:'50%', background:'#16a34a', color:'white', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, zIndex:1}}>✓</div>}
                            {p.image_url
                              ? <img src={p.image_url} alt="" style={{width:'100%', height:64, objectFit:'cover', borderRadius:5}} />
                              : <div style={{height:64, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--ink-4)', fontSize:10}}>无图</div>}
                            <div style={{fontSize:10, color:'var(--ink-2)', marginTop:4, lineHeight:1.25, maxHeight:26, overflow:'hidden'}}>{p.title || ''}</div>
                            <div style={{fontSize:9, color: saved ? '#16a34a' : 'var(--ink-4)', fontWeight:600, marginTop:2}}>{saved ? '已存入附件' : '点击存入'}</div>
                          </div>
                          );
                        })}
                      </div>
          ...
          {kind === 'aftersale' && (
            <>
              ...
              {/* 🆕 产品改进闭环(对齐「售后配件补发对接下单」表) */}
              <div style={{marginTop:4, marginBottom:14, padding:'12px', background:'#faf5ff', border:'1px solid #e9d5ff', borderRadius:8}}>
                <div style={{fontSize:12, fontWeight:700, color:'#7c3aed', marginBottom:10}}>🛠 产品改进闭环</div>
                <div style={{display:'grid', gridTemplateColumns:'2fr 1fr', gap:10, marginBottom:10}}>
                  <div>
                    <label style={{fontSize:11, fontWeight:600, color:'var(--ink-2)', display:'block', marginBottom:4}}>产品改进建议</label>
                    <textarea value={improvementSuggestion} onChange={e => setImprovementSuggestion(e.target.value)}
                      ...
                      style={{width:'100%', minHeight:56, padding:'8px 10px', border:'1px solid var(--line)', borderRadius:6, fontSize:13, fontFamily:'inherit', resize:'vertical'}} />
                  </div>
                  <div>
                    <label style={{fontSize:11, fontWeight:600, color:'var(--ink-2)', display:'block', marginBottom:4}}>产品改进状态</label>
                    <input list="improvement-status-options" value={improvementStatus} onChange={e => setImprovementStatus(e.target.value)}
                      placeholder="待跟单处理"
                      style={{width:'100%', padding:'6px 10px', border:'1px solid var(--line)', borderRadius:6, fontSize:13}} />
                    <datalist id="improvement-status-options">
                      <option value="待跟单处理" /><option value="打木箱" /><option value="改包装" /><option value="发货前检查" /><option value="已转美工修改" /><option value="已完成" />
                    </datalist>
                  </div>
                </div>
                <label style={{fontSize:11, fontWeight:600, color:'var(--ink-2)', display:'block', marginBottom:4}}>沟通图片 <span style={{color:'var(--ink-4)', fontWeight:400, fontSize:10}}>· 与工厂/供应商沟通改进的截图</span></label>
                <MultiImageUploader attachments={commImages} setAttachments={setCommImages} defaultLabel="沟通" maxSize={10} />
              </div>
            </>
          )}
...
const BatchTransferModal = ({ employees, records, user, fromUserId, setFromUserId, onClose, onTransfer }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!await wsConfirm(`确认将 ${selectedIds.size} 单工作转交给 ${employees.find(e => e.id === targetUserId)?.name}？`)) return;
...
const FollowUpModal = ({ record, onClose, onUpdate, toast, user, employees, cloudOn, readonly = false, recordEvents = null, onAddEvent, onEditEvent, onDeleteEvent }) => {
  ...
  return ReactDOM.createPortal(
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card paper p-6 w-full" style={{maxWidth:'780px'}} onClick={e => e.stopPropagation()}>
                  ...
                  return (
                    <div key={'a'+e.id} onClick={() => onEditEvent && onEditEvent('aftersale', e)}
                      ...
                      {!readonly && onDeleteEvent && (
                        <button onClick={(ev) => { ev.stopPropagation(); onDeleteEvent('aftersale', e); }} title="删除此关联事件"
                          style={{flexShrink:0, width:22, height:22, border:'none', background:'transparent', color:'#dc2626', cursor:'pointer', fontSize:15, fontWeight:700, borderRadius:4, lineHeight:1}}>×</button>
                      )}
                    </div>
                  );
                })}
                {recordEvents.refills.map(e => {
                  ...
                  return (
                    <div key={'rf'+e.id} onClick={() => onEditEvent && onEditEvent('refill', e)}
                      ...
                      {!readonly && onDeleteEvent && (
                        <button onClick={(ev) => { ev.stopPropagation(); onDeleteEvent('refill', e); }} title="删除此关联事件"
                          style={{flexShrink:0, width:22, height:22, border:'none', background:'transparent', color:'#dc2626', cursor:'pointer', fontSize:15, fontWeight:700, borderRadius:4, lineHeight:1}}>×</button>
                      )}
                    </div>
                  );
                ...
                {recordEvents.refunds.map(e => {
                  ...
                  return (
                    <div key={'rd'+e.id} onClick={() => onEditEvent && onEditEvent('refund', e)}
                      ...
                      {!readonly && onDeleteEvent && (
                        <button onClick={(ev) => { ev.stopPropagation(); onDeleteEvent('refund', e); }} title="删除此关联事件"
                          style={{flexShrink:0, width:22, height:22, border:'none', background:'transparent', color:'#dc2626', cursor:'pointer', fontSize:15, fontWeight:700, borderRadius:4, lineHeight:1}}>×</button>
                      )}
                    </div>
                  );
        ...
        {/* 大图预览 — 🆕 fix126: 行内展开(不再用带 blur 的浮层,避免被弹窗困住) */}
        {viewingImg && (
          <div style={{marginTop:12, border:'1px solid var(--line)', borderRadius:10, overflow:'hidden', background:'#111'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 10px', background:'#1c1c1e'}}>
              <span style={{color:'#d1d5db', fontSize:11}}>图片预览</span>
              <div style={{display:'flex', gap:8, alignItems:'center'}}>
                <button type="button" onClick={() => wsOpenImg(viewingImg)} style={{background:'none', border:'none', fontSize:11, color:'#7dd3fc', cursor:'pointer', padding:0}}>↗ 新标签看原图</button>
                <button type="button" onClick={() => setViewingImg(null)} style={{padding:'2px 9px', background:'rgba(255,255,255,.15)', color:'white', border:'none', borderRadius:5, cursor:'pointer', fontSize:11}}>收起 ▲</button>
              </div>
            </div>
            <div style={{textAlign:'center', padding:10}}>
              <img src={viewingImg} alt="" style={{maxWidth:'100%', maxHeight:380, objectFit:'contain', borderRadius:6}} />
            </div>
          </div>
        )}
      </div>
