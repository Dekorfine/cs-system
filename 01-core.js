...
(function () {
  ...
  function injectStyle() {
    ...
    var st = document.createElement('style'); st.id = 'ws-dialog-style';
    st.textContent =
      '.wsdlg-mask{position:fixed;inset:0;background:rgba(15,23,42,.45);z-index:2147483600;display:flex;align-items:center;justify-content:center;padding:20px;animation:wsdlgFade .12s ease}' +
      '@keyframes wsdlgFade{from{opacity:0}to{opacity:1}}' +
      '@keyframes wsdlgPop{from{transform:scale(.94);opacity:.6}to{transform:scale(1);opacity:1}}' +
      '.wsdlg-box{background:#fff;border-radius:16px;max-width:400px;width:100%;box-shadow:0 24px 70px rgba(0,0,0,.3);overflow:hidden;animation:wsdlgPop .14s ease;font-family:inherit}' +
      '.wsdlg-top{display:flex;gap:12px;align-items:flex-start;padding:22px 22px 8px}' +
      '.wsdlg-ic{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:21px;flex-shrink:0}' +
      '.wsdlg-msg{flex:1;font-size:14px;line-height:1.6;color:#1e293b;white-space:pre-wrap;word-break:break-word;padding-top:7px}' +
      '.wsdlg-ttl{font-size:15px;font-weight:700;color:#0f172a;margin-bottom:4px}' +
      '.wsdlg-inp{width:100%;margin:6px 22px 0;width:calc(100% - 44px);padding:9px 11px;border:1px solid #d1d5db;border-radius:9px;font-size:14px;font-family:inherit;box-sizing:border-box}' +
      '.wsdlg-bar{display:flex;gap:8px;justify-content:flex-end;padding:16px 22px 20px}' +
      '.wsdlg-btn{padding:9px 20px;border-radius:9px;font-size:14px;font-weight:600;cursor:pointer;border:none;font-family:inherit}' +
      '.wsdlg-ok{background:#0071e3;color:#fff}.wsdlg-ok:hover{background:#0060c0}' +
      '.wsdlg-cancel{background:#f1f5f9;color:#334155}.wsdlg-cancel:hover{background:#e2e8f0}' +
      '.wsdlg-ok.danger{background:#dc2626}.wsdlg-ok.danger:hover{background:#b91c1c}';
    document.head.appendChild(st);
  }
  function classify(msg) {
    var m = String(msg || '');
    if (/(失败|错误|不能|无法|超过|出错|异常|禁止|必须|请先|不可|拒绝|error|fail)/i.test(m)) return { ic: '⚠️', bg: '#fef2f2', color: '#dc2626', danger: true };
    if (/(成功|已完成|已保存|已删除|已更新|已发送|搞定|完成|✓|✅)/i.test(m)) return { ic: '✅', bg: '#f0fdf4', color: '#16a34a', danger: false };
    return { ic: 'ℹ️', bg: '#eff6ff', color: '#0369a1', danger: false };
  }
  ...
  function show(msg, opts) {
    ...
    var mask = document.createElement('div'); mask.className = 'wsdlg-mask';
    var box = document.createElement('div'); box.className = 'wsdlg-box';
    var top = document.createElement('div'); top.className = 'wsdlg-top';
    var ic = document.createElement('div'); ic.className = 'wsdlg-ic'; ic.style.background = info.bg; ic.textContent = info.ic;
    var msgEl = document.createElement('div'); msgEl.className = 'wsdlg-msg';
    if (opts.title) { var t = document.createElement('div'); t.className = 'wsdlg-ttl'; t.textContent = opts.title; msgEl.appendChild(t); }
    var txt = document.createElement('span'); txt.textContent = String(msg == null ? '' : msg); msgEl.appendChild(txt);
    top.appendChild(ic); top.appendChild(msgEl); box.appendChild(top);
    var input = null;
    if (mode === 'prompt') {
      input = document.createElement('input'); input.className = 'wsdlg-inp'; input.value = opts.def == null ? '' : String(opts.def);
      box.appendChild(input);
    }
    var bar = document.createElement('div'); bar.className = 'wsdlg-bar';
    var done = function (val) { if (mask.parentNode) mask.parentNode.removeChild(mask); document.removeEventListener('keydown', onKey); if (opts.onResult) opts.onResult(val); };
    if (mode !== 'alert') {
      var cancel = document.createElement('button'); cancel.className = 'wsdlg-btn wsdlg-cancel'; cancel.textContent = opts.cancelText || '取消';
      cancel.onclick = function () { done(mode === 'prompt' ? null : false); };
      bar.appendChild(cancel);
    }
    var ok = document.createElement('button'); ok.className = 'wsdlg-btn wsdlg-ok' + (info.danger ? ' danger' : ''); ok.textContent = opts.okText || (mode === 'alert' ? '知道了' : '确定');
    ok.onclick = function () { done(mode === 'prompt' ? (input ? input.value : '') : (mode === 'confirm' ? true : undefined)); };
    bar.appendChild(ok); box.appendChild(bar);
    mask.appendChild(box);
    mask.onclick = function (e) { if (e.target === mask && mode === 'alert') done(undefined); };
    function onKey(e) { if (e.key === 'Escape') done(mode === 'prompt' ? null : (mode === 'confirm' ? false : undefined)); else if (e.key === 'Enter' && mode !== 'prompt') { e.preventDefault(); ok.click(); } }
    document.addEventListener('keydown', onKey);
    document.body.appendChild(mask);
    setTimeout(function () { (input || ok).focus(); }, 30);
  }
...
function slimRecordsForCache(records) {
  const stripShots = arr => (arr || []).map(s => (s && s.data) ? { ...s, data: '', _cloud: true } : s);
  return (records || []).map(r => {
    if (!r || typeof r !== 'object') return r;
    const o = { ...r };
    if (Array.isArray(o.screenshots)) o.screenshots = stripShots(o.screenshots);
    if (Array.isArray(o.followUps)) o.followUps = o.followUps.map(f => (f && Array.isArray(f.screenshots)) ? { ...f, screenshots: stripShots(f.screenshots) } : f);
    return o;
  });
}
...
const CLOUD = {
  ...
  async list(table, opts = {}) {
    ...
    const HEAVY = { chargebacks: 'id,amount,currency,status,site,order_no,customer_name,customer_email,platform,reason,reason_detail,resolution,notes,deadline,opened_at,created_at,updated_at,created_by,created_by_name,assigned_to,assigned_to_names,assigned_at,resolved_at,cs_fault,cs_fault_emp,deleted' };
    let sel = opts.select || (!opts.full && HEAVY[table]) || '*';
    const runQuery = async (selStr) => {
      let q = this.client.from(table).select(selStr);
    ...
    try {
      let { data, error } = await runQuery(sel);
      if (error && sel !== '*') {  // 精简列出错(列名不符等)→ 回退全列,绝不让列表崩
  ...
  _missingColumns: (function () {
    try {
      const s = (typeof sessionStorage !== 'undefined') && sessionStorage.getItem('cloud_missing_cols');
      if (s) { const o = JSON.parse(s); const r = {}; Object.keys(o).forEach(k => { r[k] = new Set(o[k] || []); }); return r; }
    } catch (e) {}
    return {};
  })(),  // { table: Set<column> }
  _retryStats: { total: 0, byTable: {} },  // 🆕 fix140: 诊断 — 控制台看 CLOUD._retryStats 即知重试频率
  _persistMissing() {
    try {
      if (typeof sessionStorage === 'undefined') return;
      const o = {}; Object.keys(this._missingColumns).forEach(k => { o[k] = [...this._missingColumns[k]]; });
      sessionStorage.setItem('cloud_missing_cols', JSON.stringify(o));
    } catch (e) {}
  },
  clearMissingColumns(table) {  // 🆕 fix140: 主管补列后调用 CLOUD.clearMissingColumns() 清空,恢复写入该列
    if (table) delete this._missingColumns[table]; else this._missingColumns = {};
    try { if (typeof sessionStorage !== 'undefined') sessionStorage.removeItem('cloud_missing_cols'); } catch (e) {}
  },
    ...
    try {
      ...
      if (error) {
        ...
        if (colMatch && _retryCount < 5) {
          ...
          this._persistMissing();  // 🆕 fix140: 持久化到本会话,重载后直接剥离不再重试
          this._retryStats.total++;  // 🆕 fix140: 诊断计数
          this._retryStats.byTable[table] = (this._retryStats.byTable[table] || 0) + 1;
...
const INITIAL_EMPLOYEES = [
  ...
  { id:'u_hr_zhaoxin', name:'赵欣', alias:'', sites:'人事', role:'hr', password:'hr2026', title:'人事 · HR', department:'人事' },
...
async function prReviewsList(opts = {}) {
  const c = getCdmClient(); if (!c) return null;
  try {
    let q = c.from('product_reviews').select('*');
    if (opts.order) q = q.order(opts.order.col, { ascending: opts.order.asc !== false });
    if (opts.limit) q = q.limit(opts.limit);
    const { data, error } = await q;
    if (error) { console.error('[product_reviews] list 失败', error); return null; }
    return data || [];
  } catch (e) { console.error('[product_reviews] list 异常', e); return null; }
}
async function prReviewUpsert(row) {
  const c = getCdmClient(); if (!c) return null;
  try { const { data, error } = await c.from('product_reviews').upsert(row).select(); if (error) { console.error('[product_reviews] upsert 失败', error); return null; } return data; }
  catch (e) { console.error('[product_reviews] upsert 异常', e); return null; }
}
async function prReviewInsert(rows) {
  const c = getCdmClient(); if (!c) throw new Error('共享库(消息总线)未初始化');
  const { error } = await c.from('product_reviews').insert(rows); if (error) throw error;
}
...
async function publishMyStaff(staffList, updatedBy) {
  ...
  const rows = staffList.filter(s => s.role !== 'hr').map((s, i) => {
    const cn = s.name || '';                       // 中文名
    const en = s.alias || '';                      // 英文名(alias)
    const displayName = (en && cn && en !== cn) ? `${en}(${cn})` : (en || cn || s.name);  // 🆕 双显:Nicole(李彬桦)
    // 🆕 fix116: HR(赵欣)由 HR/跟单侧统一发布(hr_zhaoxin · system='hr'),客服侧过滤掉 role='hr' 不重复发
    const sysVal = ORG_SYSTEM;
    const idVal = `${ORG_SYSTEM}_${s.id}`;
    return {
      id: idVal,
      staff_id: s.id,
      name: displayName,                            // name 也存双显,兼容旧读法
      chinese_name: cn,                             // 🆕
      english_name: en,                             // 🆕
  ...
  if (error) throw error;
  _orgDirCache = { ts: 0, data: null };  // 🆕 fix140: 发布后失效缓存,下次 load 拿最新
  return rows.length;
}
...
let _orgDirCache = { ts: 0, data: null };
const ORG_DIR_TTL = 5 * 60 * 1000;
async function loadOrgDirectory(force = false) {
  const now = Date.now();
  if (!force && _orgDirCache.data && (now - _orgDirCache.ts) < ORG_DIR_TTL) return _orgDirCache.data;
  const client = getCdmClient();
  if (!client) return _orgDirCache.data || [];
  const { data, error } = await client.from('org_directory').select('*').order('sort_order', { ascending: true });
  if (error) { console.error('[org_directory] 读取失败', error); return _orgDirCache.data || []; }
  const mapped = (data || []).map(r => ({
    id: r.id, staffId: r.staff_id,
    name: r.display_name || r.name,                 // 🆕 优先双显名
    displayName: r.display_name || r.name,          // 🆕
    chineseName: r.chinese_name || '', englishName: r.english_name || '',
    system: r.system,
    role: r.role, department: r.department, active: r.active !== false,
    sortOrder: r.sort_order,
  }));
  _orgDirCache = { ts: now, data: mapped };  // 🆕 fix140
  return mapped;
}
...
function accountFor(e) {
  ...
  const a = (e && e.alias || '').trim().toLowerCase().replace(/\s+/g, '');
  return a || String(e && e.id || '').replace(/^u_/, '').toLowerCase();
}
function empToAccountRow(e) {
  return {
    id: e.id, account: accountFor(e), password: e.password || '123456',
    name: e.name || '', alias: e.alias || '', sites: e.sites || '', role: e.role || 'staff',
    title: e.title || '', team: e.team || '', hide_from_list: !!e.hideFromList,
    active: e.active !== false && !e.disabled, updated_at: new Date().toISOString(),
  };
}
function accountRowToEmp(r) {
  return {
    id: r.id, account: r.account, password: r.password, name: r.name, alias: r.alias,
    sites: r.sites, role: r.role, title: r.title || '', team: r.team || '',
    hideFromList: !!r.hide_from_list, disabled: r.active === false,
  };
}
// 拉云端账号:成功→数组;失败(表不存在/未连)→null(调用方回退本地)
async function loadCloudAccounts() {
  const rows = await CLOUD.list('cs_accounts', { limit: 1000, full: true });
  if (!rows) return null;
  return rows.map(accountRowToEmp);
}
async function saveCloudAccount(e) { try { return await CLOUD.upsert('cs_accounts', empToAccountRow(e)); } catch (x) { return null; } }
async function deleteCloudAccount(id) { try { if (CLOUD.client) await CLOUD.client.from('cs_accounts').delete().eq('id', id); } catch (x) {} }
// 首次播种:把名单批量写入空表(直接用 client 批量 upsert)
async function seedCloudAccounts(list) {
  try {
    if (!CLOUD.client) return false;
    const rows = (list || []).map(empToAccountRow);
    const { error } = await CLOUD.client.from('cs_accounts').upsert(rows, { onConflict: 'id' });
    if (error) { console.error('[账号播种] 失败', error); return false; }
    console.log('[账号播种] 已写入', rows.length, '个账号到云端 cs_accounts');
    return true;
  } catch (x) { console.error('[账号播种] 异常', x); return false; }
}
if (typeof window !== 'undefined') {
...
const ORDERS_KEY_WS = 'sb_publishable_dFjk1WN_Hc0Te6IhXZysZg_SXvKQU4C';
const EDGE_BASE_WS = 'https://pyfmuknvjqfwcqvbrsvw.supabase.co/functions/v1';
const PREFIX_TO_DOMAIN_WS = {
  V:'vakkerlighting.myshopify.com', VK:'vakkerlighting.myshopify.com', K:'vakkerlighting.myshopify.com',
  PL:'vkfrench.myshopify.com', DF:'dekorfine.myshopify.com', DC:'docolight.myshopify.com',
  MH:'vkwholesale.myshopify.com', LS:'docolamp.myshopify.com', RD:'vakkerge.myshopify.com',
  RS:'decormote.myshopify.com', JD:'janedecor.myshopify.com', MJ:'janedecor.myshopify.com', ML:'mooielight.com',
};
function wsGuessDomain(no) {
  const m = String(no || '').toUpperCase().match(/^([A-Z]+)/);
  if (!m) return null;
  const p = m[1];
...
const PREFIX_TO_SITE_WS = {
  VK:'vakkerlight', V:'vakkerlight', K:'vakkerlight', RD:'Radium', ML:'Mooielight', MO:'Mooielight',
  MJ:'Mooijane', JD:'Mooijane', DC:'Docos', DF:'dekorfine', LS:'Lumioshine', RS:'Rayonshine',
  MH:'Mooiehome', PL:'Pinlighting',
};
...
function wsOpenImg(src) {
  ...
  try {
    if (/^data:/i.test(src)) {
      const arr = String(src).split(','); const mime = (arr[0].match(/:(.*?);/) || [])[1] || 'image/png';
...
try { window.wsOpenImg = wsOpenImg; } catch (e) {}
...
function imgDisplaySrc(v) {
  ...
  if (typeof v === 'string') {
    const s = v.trim();
    if (!s) return '';
    if (s[0] === '{' || s[0] === '[') { try { return imgDisplaySrc(JSON.parse(s)); } catch (e) { return s; } }
    return s;  // URL 或 dataURL 原样
  }
  if (Array.isArray(v)) return v.length ? imgDisplaySrc(v[0]) : '';
  if (typeof v === 'object') {
    const direct = v.url || v.dataUrl || v.dataURL || v.data || v.image_url || v.imageUrl || v.src || v.publicUrl;
...
function attMimeKind(a) {
  const m = (a && (a.mime || a.type)) || '';
  if (/^image\//i.test(m)) return 'image';
  const src = imgDisplaySrc(a);
...
function attName(a) {
  if (a && typeof a === 'object') return a.name || a.filename || a.title || '附件';
  return '附件';
}
try { window.imgDisplaySrc = imgDisplaySrc; window.attMimeKind = attMimeKind; } catch (e) {}

// 缩略图行:image/* 铺 48×48,其它显 📄 文件名。点击 onPreview(全屏看大图),stopPropagation 防触发卡片点击。
const AttachThumbs = ({ files, size = 48, max = 0, onPreview }) => {
  const list = Array.isArray(files) ? files.filter(Boolean) : (files ? [files] : []);
  if (!list.length) return null;
  const show = max > 0 ? list.slice(0, max) : list;
  const more = max > 0 ? Math.max(0, list.length - max) : 0;
  const preview = onPreview || (typeof window !== 'undefined' && window.__setPreviewImg) || (() => {});
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 4 }}>
      {show.map((a, i) => {
        const kind = attMimeKind(a);
        if (kind === 'image') {
          const src = imgDisplaySrc(a);
          return (
            <img key={i} src={src} alt="" loading="lazy"
              ...
              title="点击看大图"
              style={{ width: size, height: size, objectFit: 'cover', borderRadius: 6, border: '1px solid var(--line, #e5e7eb)', cursor: 'zoom-in', background: 'var(--bg-2, #f3f4f6)' }} />
          );
        }
        return (
          <a key={i} href={imgDisplaySrc(a) || '#'} target="_blank" rel="noopener"
            onClick={(e) => e.stopPropagation()} title={attName(a)}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 4, height: size, padding: '0 8px', borderRadius: 6, border: '1px solid var(--line, #e5e7eb)', fontSize: 11, color: 'var(--ink-2, #374151)', textDecoration: 'none', maxWidth: 140, overflow: 'hidden', whiteSpace: 'nowrap' }}>
            📄 <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{attName(a)}</span>
          </a>
        );
      })}
      {more > 0 && (
        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size, height: size, borderRadius: 6, border: '1px dashed var(--line, #e5e7eb)', fontSize: 11, color: 'var(--ink-3, #6b7280)' }}>+{more}</span>
      )}
    </div>
  );
};
try { window.AttachThumbs = AttachThumbs; } catch (e) {}

// App 级单例:全屏预览弹窗。点击任意处关闭;支持 dataURL/URL/对象,均过 imgDisplaySrc。
const ImgPreviewModal = ({ img, onClose }) => {
  if (!img) return null;
  const src = imgDisplaySrc(img);
  return (
    <div onClick={onClose}
      style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.82)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out', padding: 24 }}>
      <img src={src} alt=""
        onClick={(e) => e.stopPropagation()}
        style={{ maxWidth: '94vw', maxHeight: '90vh', objectFit: 'contain', borderRadius: 8, boxShadow: '0 12px 48px rgba(0,0,0,0.5)' }} />
      <button onClick={onClose}
        style={{ position: 'fixed', top: 16, right: 20, background: 'rgba(255,255,255,0.92)', border: 'none', borderRadius: 20, width: 40, height: 40, fontSize: 20, cursor: 'pointer', lineHeight: '40px' }}>✕</button>
      <button onClick={(e) => { e.stopPropagation(); wsOpenImg(src); }}
        style={{ position: 'fixed', bottom: 18, right: 20, background: 'rgba(255,255,255,0.92)', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 12, cursor: 'pointer' }}>↗ 原图新标签</button>
    </div>
  );
};
try { window.ImgPreviewModal = ImgPreviewModal; } catch (e) {}

// ====== fix143: 流程步骤条(对齐拍摄 worktrack 清晰展示)======
// 人名哈希成固定颜色 —— 同一个人到处同色,一眼认人。
const NAME_PALETTE = ['#dc2626','#ea580c','#d97706','#65a30d','#059669','#0d9488','#0891b2','#2563eb','#4f46e5','#7c3aed','#9333ea','#c026d3','#db2777','#e11d48','#b45309','#0f766e'];
...
function fmtStepDate(v) {
  ...
  const s = String(v); return s.length >= 10 ? s.slice(5, 10) : s;
}
try { window.nameColor = nameColor; window.fmtStepDate = fmtStepDate; } catch (e) {}
...
const ProcessStepper = ({ steps }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', marginTop: 8, width: '100%' }}>
      {steps.map((s, i) => (
        <React.Fragment key={s.id || i}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', flexShrink: 0, width: 84 }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#fff',
              background: s.done ? s.color : '#e7e5e4', boxShadow: s.done ? `0 3px 8px ${s.color}66` : 'none' }}>
              {s.done ? s.icon : <span style={{ color: '#a8a29e', fontSize: 15, fontWeight: 700 }}>{i + 1}</span>}
            </div>
            <div style={{ fontSize: 11, marginTop: 6, lineHeight: 1.2, fontWeight: 700, color: s.done ? '#1c1917' : '#a8a29e' }}>{i + 1}. {s.label}</div>
            {s.who
              ? (s.done
                  ? <div style={{ fontSize: 11, lineHeight: 1.2, fontWeight: 700, color: nameColor(s.who), maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={s.who}>{s.who}</div>
                  : <div style={{ fontSize: 10, lineHeight: 1.2, color: '#a8a29e', maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={'待 ' + s.who}>待 {s.who}</div>)
              : <div style={{ fontSize: 10, lineHeight: 1.2, color: '#d6d3d1' }}>—</div>}
            {s.when && <div style={{ fontSize: 10, color: '#a8a29e', marginTop: 1, lineHeight: 1.3 }}>{s.when}</div>}
          </div>
          {i < steps.length - 1 && (
            <div style={{ height: 4, borderRadius: 2, flex: '1 1 0%', minWidth: 14, marginTop: 20,
...
try { window.ProcessStepper = ProcessStepper; } catch (e) {}
...
function wsPickImg(li) {
  if (!li) return '';
  const cands = [li.image_url, li.imageUrl, li.img, li.img_url, li.thumbnail, li.product_image, li.featured_image, li.image];
  for (const c of cands) {
    if (typeof c === 'string' && c.trim()) return c.trim();
    if (c && typeof c === 'object') { const s = c.src || c.url || c.image_url; if (s) return s; }
  }
  return '';
}
// 订单后台搜索链接(无需额外请求,直接拼 Shopify admin 搜索)
function wsOrderAdminUrl(no) {
  const d = wsGuessDomain(no);
  if (!d || /mooielight/i.test(d)) return '';
  return `https://${d}/admin/orders?query=${encodeURIComponent(wsNormKey(no))}`;
}
const _wsOrderCache = {};
async function wsFetchOrderProducts(orderNo) {
  const base = wsNormKey(orderNo);
  if (!base) return null;
  const domain = wsGuessDomain(base);
  if (!domain) return null;
  const ck = domain + '|' + base;
  const hit = _wsOrderCache[ck];
  if (hit && Date.now() - hit.ts < 5 * 60 * 1000) return hit.v;
  const isWoo = /mooielight/i.test(domain);
  try {
    for (const nm of [base, '#' + base]) {
      const body = isWoo
        ? { store_id:'mooielight', action:'list_orders', params:{ search:nm, per_page:5 } }
        : { shop:domain, action:'list_orders', params:{ name:nm, status:'any', limit:5, auto_save:false } };
      const res = await fetch(EDGE_BASE_WS + '/' + (isWoo ? 'woo-api' : 'shopify-api'), {
        method:'POST',
        headers:{ 'Authorization':'Bearer ' + ORDERS_KEY_WS, 'apikey':ORDERS_KEY_WS, 'Content-Type':'application/json' },
      ...
      if (json && json.ok !== false && Array.isArray(arr) && arr.length) {
        ...
        const products = li.map(it => ({ title: it.title || it.name || '', image_url: wsPickImg(it), quantity: it.quantity || 1 }))
          .filter(p => p.image_url || p.title);
        const oid = o.id || o.shopify_order_id;
        const orderUrl = oid && !isWoo ? `https://${domain}/admin/orders/${oid}` : wsOrderAdminUrl(orderNo);
        // 🆕 fix119: 一并带回客户邮箱/名字 + 金额 + 币种(供表单自动填充)
        const cust = o.customer || {};
        const bill = o.billing || (o.raw_payload && o.raw_payload.billing) || {};
        const email = o.email || cust.email || bill.email || '';
        const cname = [cust.first_name || bill.first_name, cust.last_name || bill.last_name].filter(Boolean).join(' ').trim()
          || o.name || [bill.first_name, bill.last_name].filter(Boolean).join(' ').trim() || '';
        const total = o.current_total_price || o.total_price || o.total || o.total_price_set?.shop_money?.amount || '';
        const currency = o.currency || o.presentment_currency || (o.total_price_set?.shop_money?.currency_code) || 'USD';
        const country = (o.shipping_address && o.shipping_address.country_code) || (cust.default_address && cust.default_address.country_code) || (bill.country || '') || '';
        const v = { products, orderUrl, domain, email, customerName: cname, total: total ? String(total) : '', currency, country };
        _wsOrderCache[ck] = { ts: Date.now(), v };
        return v;
      }
    }
  } catch (e) { console.warn('wsFetchOrderProducts', orderNo, e); }
  const v = { products: [], orderUrl: wsOrderAdminUrl(orderNo), domain };
  _wsOrderCache[ck] = { ts: Date.now(), v };
  return v;
}
...
async function uploadAttachmentToWtkpi(file) {
  ...
  // 🆕 fix106: 之前视频也被强行走 canvas 图片压缩 → new Image() 无法加载视频 → 失败。现在只有图片压缩,视频原样传。
  const payload = isImage ? await compressImageForUpload(file, 1600, 0.85) : file;
  const ext = (file.name || (isImage ? 'img.png' : 'file.bin')).split('.').pop() || (isImage ? 'png' : 'bin');
  const path = `cs-requests/${Date.now()}-${crypto.randomUUID().slice(0,8)}.${ext}`;
  const { error } = await client.storage.from('attachments').upload(path, payload, {
    upsert: false,
  ...
  return {
    ...
    url: publicUrl,
    mime: payload.type || file.type || (isImage ? 'image/jpeg' : 'application/octet-stream'),  // 🆕 fix72: 三方统一附件结构需要 mime
...
async function listPhotoRequests(opts = {}) {
  const client = getWtkpiClient();
  if (!client) return [];
...
const REVIEW_DOMAINS = ['vakkerlight.com', 'radium.com', 'pinlighting.com', 'dekorfine.com', 'docos.us', 'mooiehome.com', 'lumioshine.com', 'rayonshine.com', 'mooijane.com', 'mooielight.com'];
// 订单代码 / 品牌名 / 内部店铺名 → 公开域名(别名,大小写无关)
const SITE_ALIAS_DOMAIN = {
  vk: 'vakkerlight.com', v: 'vakkerlight.com', k: 'vakkerlight.com', vakkerlight: 'vakkerlight.com', vakkerlighting: 'vakkerlight.com',
  rd: 'radium.com', radium: 'radium.com', radilum: 'radium.com', vakkerge: 'radium.com',
  pl: 'pinlighting.com', pinlighting: 'pinlighting.com', vkfrench: 'pinlighting.com',
  df: 'dekorfine.com', dekorfine: 'dekorfine.com',
  dc: 'docos.us', docos: 'docos.us', docolight: 'docos.us', decora: 'docos.us',
  mh: 'mooiehome.com', mooiehome: 'mooiehome.com', vkwholesale: 'mooiehome.com',
  ls: 'lumioshine.com', lumioshine: 'lumioshine.com', docolamp: 'lumioshine.com',
  rs: 'rayonshine.com', rayonshine: 'rayonshine.com', decormote: 'rayonshine.com',
  mj: 'mooijane.com', jd: 'mooijane.com', mooijane: 'mooijane.com', janedecor: 'mooijane.com', myjewelry: 'mooijane.com',
  ml: 'mooielight.com', mo: 'mooielight.com', mooielight: 'mooielight.com',
};
// 把任意写法(产品链接 / 代码 / 品牌 / 域名)规范成公开域名
const siteToDomain = (raw, url) => {
  if (url) {
    try {
      const h = new URL(url).hostname.toLowerCase().replace(/^www\./, '');
      const hit = REVIEW_DOMAINS.find(d => h === d || h.endsWith('.' + d));
      if (hit) return hit;
      const key = Object.keys(SITE_ALIAS_DOMAIN).find(k => k.length >= 3 && h.includes(k));
      if (key) return SITE_ALIAS_DOMAIN[key];
    } catch (e) {}
  }
  const r = (raw || '').trim().toLowerCase().replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/.*$/, '');
  if (!r) return '';
...
const LoginScreen = ({ employees, onLogin }) => {
  ...
  useEffect(() => { setTimeout(() => acctRef.current && acctRef.current.focus(), 120); }, []);

  const tryLogin = () => {
    const acc = account.trim().toLowerCase();
    if (!acc) { setError('请输入账号'); return; }
    // 账号匹配(含隐藏老板):account 字段 / 英文名 / id 去 u_ 前缀,全部小写比对
    const sel = employees.find(e => accountFor(e) === acc)
             || employees.find(e => (e.alias || '').trim().toLowerCase() === acc)
             || employees.find(e => String(e.id || '').replace(/^u_/, '').toLowerCase() === acc);
    if (!sel) { setError('账号不存在 — 请联系主管确认你的账号'); return; }
    if ((sel.password || '') !== password) {
      console.warn('[登录失败] 账号:', acc, '· 密码长度', password.length);
      setError('密码不正确 — 如忘记请联系主管重设'); return;
    }
  ...
  return (
    <div className="login-bg" data-login-screen>
      <div className="login-card fade-in" style={{maxWidth:420}}>
        <div style={{padding:'48px 44px 22px', textAlign:'center', borderBottom:'1px solid var(--line)'}}>
          <div className="font-display" style={{fontSize:'38px', fontWeight:600, letterSpacing:'-.022em', marginBottom:'6px'}}>统一工作台</div>
          <div style={{color:'var(--ink-3)', fontSize:'15px', fontWeight:400}}>输入账号和密码登录</div>
        </div>
        <div style={{padding:'30px 44px 34px'}}>
          <label style={{display:'block', fontSize:12, fontWeight:600, color:'var(--ink-2)', marginBottom:6}}>账号</label>
          <input ref={acctRef} value={account} autoCapitalize="off" autoCorrect="off" spellCheck={false}
            onChange={e => { setAccount(e.target.value); setError(''); }}
            onKeyDown={e => { if (e.key === 'Enter') { const p = document.getElementById('login-pwd'); if (p) p.focus(); } }}
            placeholder="你的账号(英文名小写,如 nicole)"
            style={{fontSize:15, padding:'12px 14px', borderRadius:10, width:'100%', border:'1px solid var(--line)', outline:'none', fontFamily:'inherit', marginBottom:16, boxSizing:'border-box'}} />
          <label style={{display:'block', fontSize:12, fontWeight:600, color:'var(--ink-2)', marginBottom:6}}>密码</label>
          <input id="login-pwd" type="password" value={password}
            onChange={e => { setPassword(e.target.value); setError(''); }}
            onKeyDown={e => e.key === 'Enter' && tryLogin()}
            placeholder="输入密码"
            className={error ? 'input-error' : ''}
            style={{fontSize:15, padding:'12px 14px', borderRadius:10, width:'100%', border:'1px solid var(--line)', outline:'none', fontFamily:'inherit', boxSizing:'border-box'}} />
          {error && (
            <div className="fade-in" style={{marginTop:14, padding:'10px 14px', borderRadius:10, background:'#fef2f2', border:'1px solid #fca5a5', fontSize:13, color:'#991b1b', fontWeight:600, display:'flex', alignItems:'center', gap:8}}>
              <span style={{fontSize:16}}>⚠️</span><span>{error}</span>
            </div>
          )}
          <button className="btn-pri" onClick={tryLogin}
            style={{width:'100%', marginTop:20, padding:'13px 24px', fontSize:15, fontWeight:600}}>
            登录 →
          </button>
          <div style={{marginTop:16, fontSize:12, color:'var(--ink-4)', textAlign:'center', lineHeight:1.7}}>
            账号一般是你的英文名小写(如 <b>nicole</b> / <b>sally</b>)。<br/>找不到账号或忘记密码?联系主管。
          </div>
        </div>
      </div>
    </div>
