...
var QuoteModule = function QuoteModule(_ref) {
  ...
  // 🆕 fix236:带上要打开的报价(从客服「📄」跳转来),首帧 iframe 就定位到该报价
  var sameDirUrl = "quotation.html#user=".concat(userParam) + (openRef ? "&open=".concat(encodeURIComponent(openRef)) : '');
  var _useState = useState(false),
    _useState2 = _slicedToArray(_useState, 2),
    openExternal = _useState2[0],
    setOpenExternal = _useState2[1];
  var iframeRef = React.useRef(null);
  // openRef 变化(iframe 已挂载)时,实时通知 iframe 打开该报价
  useEffect(function () {
    if (!openRef) return;
...
var ChargebackReminderBanner = function ChargebackReminderBanner(_ref18) {
  ...
  useEffect(function () {
    ...
    var criticalOnes = urgent.filter(function (c) {
      ...
      return ['overdue', 'today', 'critical', 'urgent'].includes((_getChargebackUrgency = getChargebackUrgency(c)) === null || _getChargebackUrgency === void 0 ? void 0 : _getChargebackUrgency.level);
    });
