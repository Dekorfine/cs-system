...
const BriefingsModule = ({ user, employees, toast, cloudOn }) => {
  ...
  const handleDelete = async (id) => {
    if (!await wsConfirm('确认删除此会议纪要？')) return;
...
const GlobalSearch = ({ open, onClose, user, employees, records, setActiveTab, onJumpToRecord }) => {
    ...
    (extraData.aftersales || []).forEach(a => {
      ...
      if (hay.includes(q)) {
        ...
        all.push({
          ...
          status: status?.label,
          target: { tab:'events', subTab:'aftersales', recordId: a.id },
          data: a,
        });
      }
    });
    ...
    (extraData.refills || []).forEach(r => {
      ...
      if (hay.includes(q)) {
        ...
        all.push({
          ...
          extra: r.notes,
          status: status?.label,
          target: { tab:'events', subTab:'refills', recordId: r.id },
          data: r,
        });
      }
    });
    ...
    (extraData.refunds || []).forEach(r => {
      ...
      if (hay.includes(q)) {
        ...
        all.push({
          ...
          status: status?.label,
          target: { tab:'events', subTab:'refunds', recordId: r.id },
          data: r,
        });
      }
    });
    ...
    (extraData.chargebacks || []).filter(c => !c.deleted).forEach(c => {
      ...
      if (hay.includes(q)) {
        ...
        all.push({
          ...
          target: { tab:'chargebacks', recordId: c.id },
          data: c,
        });
      }
    });
  ...
  const handleClick = (item) => {
    if (item.target && item.target.tab) {
      if (onJumpToRecord) onJumpToRecord(item.target);
      else setActiveTab(item.target.tab);
    }
    onClose();
  };
...
const ChargebackReminderBanner = ({ user, employees, onJumpTo }) => {
  ...
  useEffect(() => {
    ...
    const criticalOnes = urgent.filter(c => ['overdue','today','critical','urgent'].includes(getChargebackUrgency(c)?.level));
...
const WorkSnapshotPanel = ({ user, employees, records, onJumpTo }) => {
  ...
  const load = async () => {
    ...
    try {
      // 加载阈值配置
      try {
        const { data: th } = await CLOUD.client.from('system_settings').select('*').eq('key', 'alert_thresholds').maybeSingle();
